...
void PathConstrainer::Apply( Property target, Property source, const Vector2& range)
{
  ...
  if( propertyType == Dali::Property::VECTOR3)
  {
    ...
    Dali::Constraint constraint = Dali::Constraint::New<Vector3>( target.object, target.propertyIndex, PathConstraintFunctor( mPath, range ) );
  ...
  else if( propertyType == Dali::Property::ROTATION )
  {
    ...
    Dali::Constraint constraint = Dali::Constraint::New<Quaternion>( target.object, target.propertyIndex, PathConstraintFunctor( mPath, range, mForward) );
    constraint.AddSource( Dali::Source(source.object, source.propertyIndex ) );
