...
enum ResourceTypeId
{
  /** A blob of image data with dimension and type metadata.*/
  ResourceImageData,
  ResourceNativeImage,
  ResourceTargetImage,
  ResourceShader,
  ...
  ResourceMesh,
  ResourceText,
  /** Used to pass through a request for a GLES texture to be allocated on the render thread. */
  ResourceTexture,
  /** @deprecated Used in Core to tag Bitmaps that belong to BitmapImages, but not used in adaptor so doesn't actually belong here.
   * */
  ResourceAppBitmap
};

/**
 * A ResourceType is just a data bucket. Dispatch on the ResourceTypeId to
 * know exactly which kind of resource request is associated with a particular
 * instance of one.
...
struct DALI_IMPORT_API ImageResourceType : public ResourceType
{
  ...
  {
    DALI_ASSERT_DEBUG( typeId == ResourceTexture || typeId == ResourceImageData || typeId == ResourceNativeImage || typeId == ResourceTargetImage );
  }
 ...
 * from PlatformAbstraction::LoadResource()
 * @note If the base class were not pure, we could just use that.
 */
