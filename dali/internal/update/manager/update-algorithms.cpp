...
inline void UpdateNodeTransformValues( Node& node, TxManager* txManager, int& nodeDirtyFlags, BufferIndex updateBufferIndex )
{
  ...
  if( nodeDirtyFlags & TransformFlag )
  {
    txManager->SetTransform( node.GetTxId(), node.GetPosition( updateBufferIndex), node.GetScale(updateBufferIndex), node.GetSize(updateBufferIndex), node.GetOrientation(updateBufferIndex) );
                               ...
                               RenderQueue& renderQueue )
{
  ...
  UpdateNodeTransformValues( rootNode, txManager, nodeDirtyFlags, updateBufferIndex );
