  ...
  const RenderDataProvider* dataProvider )
{
  ...
  for( unsigned int i=0; i<vertexBuffers.Count(); ++i)
  {
    ...
    RenderPropertyBuffer* propertyBuffer = new RenderPropertyBuffer( *vertexBuffer, GpuBuffer::ARRAY_BUFFER );
  ...
  if( indexBuffer )
  {
    mIndexBuffer = new RenderPropertyBuffer( *indexBuffer, GpuBuffer::ELEMENT_ARRAY_BUFFER );
