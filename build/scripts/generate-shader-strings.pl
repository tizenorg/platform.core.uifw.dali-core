...
sub GenerateStringsFromFile
{
  ...
  while(<MEM>)
  {
    ...
    if ($state == 0)
    {
      ...
      {
        $state = 1;
        print $headerFile "extern const char* ${shadername}Vertex;\n";
        print $sourceFile "extern const char* ${shadername}Vertex(\n";
      }
      elsif (/<FragmentShader>/)
      {
        $state = 1;
        print $headerFile "extern const char* ${shadername}Fragment;\n";
        print $sourceFile "extern const char* ${shadername}Fragment(\n";
      }
    }
    elsif ($state == 1)
    {
