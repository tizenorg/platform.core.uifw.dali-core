...
bool AreAllWhiteSpaces( const std::vector<uint32_t>& text, std::size_t from, std::size_t numberOfCharacters )
{
  for( std::vector<uint32_t>::const_iterator it = text.begin() + from, endIt = text.begin() + ( from + numberOfCharacters ); it != endIt; ++it )
  {
...
void MergeWhiteSpaceStyles( Integration::Text& textToProcess )
{
  ...
  std::vector<Integration::FontPerCharacter*> fontPerCharacters = textToProcess.mFontPerCharacters;
  ...
  for( std::vector<Integration::FontPerCharacter*>::const_iterator it = fontPerCharacters.begin(), endIt = fontPerCharacters.end(); it != endIt; ++it )
  {
    ...
    if( isWhiteSpace )
    {
      ...
      else
      {
        ...
        textToProcess.mFontPerCharacters.push_back( fontPerCharacter );
...
void TextProcessor::ProcessMetrics( Dali::StyledText& styledText,
                                    ...
                                    std::vector<TextProcessor::FontInfo>& fontInfoForCharacters )
{
  ...
  const TextArray& textArray = styledText.GetText().GetImplementation().GetTextArray();
  ...
  std::vector<Integration::FontPerCharacter*> fontPerCharacters;
  ...
  std::size_t index = 0u;
  for( TextArray::const_iterator it = textArray.begin(), endIt = textArray.end(); it != endIt; ++it, ++index )
  {
    TextStyle style;
        ...
        ( fabsf( fontSize - fontPerCharacter->mFontPointSize ) > Math::MACHINE_EPSILON_1000 ) )
    {
      ...
      {
        fontPerCharacters.push_back( fontPerCharacter );
  ...
  metrics.resize( textArray.size() );
  fontInfoForCharacters.resize( fontPerCharacters.size() );

  const Font defaultFont = Font::New();
  std::size_t from = 0u;

  std::vector<Integration::FontPerCharacter*>::iterator fontIt, fontEndIt;
       ...
       ++fontIt, ++infoIt )
  {
        ...
        !fontPerCharacter->mIsFontPointSizeDefault )
    {
      ...
      styledText.SetStyle( style, from, from + fontPerCharacter->mNumberOfCharacters - 1u, mask );
  ...
  DALI_ASSERT_DEBUG( textArray.size() == metrics.size() );
  DALI_ASSERT_DEBUG( fontPerCharacters.size() == fontInfoForCharacters.size() );
  // std::cout << "<--TextProcessor::ProcessMetrics" << std::endl;
}

void TextProcessor::ProcessStyles( const Dali::StyledText& styledText,
                                   Vector<StyledCharacters*>& styleInfoForCharacters )
{
  ...
  for( std::vector<std::size_t>::const_iterator it = styleChangePositions.begin(), endIt = styleChangePositions.end(); it != endIt; ++it )
  {
    ...
    if( previousStyle != currentStyle )
    {
      ...
      {
        std::size_t numberOfCharacters = currentPosition - previousPosition;
    ...
    std::vector<std::size_t>::const_iterator nextIt = it + 1u;
    ...
    if( checkNextPosition )
    {
      ...
      if( previousStyle != nextStyle )
      {
        ...
        std::size_t numberOfCharacters = nextPosition - previousPosition;
