...
void DebugBoundingBox(Context& context, RenderDataProvider& dataProvider, const Matrix& model, const Matrix& viewMatrix, const Matrix& projectionMatrix)
{
  ...
  // Debug bounding box
  Vector3 halfExtent = dataProvider.GetHalfExtent();
  //Vector3 center = mDataProvider.GetCenter(); // Assume MVP using same center

  const unsigned int numPoints=8;
  GLfloat vertices[numPoints*3] = {
    -halfExtent.x, -halfExtent.y, 0.0f,
    -halfExtent.x,  halfExtent.y, 0.0f,
  ...
  if( mvpLoc != Program::UNIFORM_UNKNOWN )
  {
    ...
    Quaternion rotation;
    Vector3 scale;
    model.GetTransformComponents( position, rotation, scale );
    Matrix newModel(false);
    newModel.SetTransformComponents( scale, Quaternion::IDENTITY, position );
    Matrix::Multiply( mvp, newModel, viewMatrix );
    Matrix::Multiply( mvp, mvp, projectionMatrix );
