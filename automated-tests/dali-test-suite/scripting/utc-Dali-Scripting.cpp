...
TEST_FUNCTION( UtcDaliScriptingCreatePropertyMap, POSITIVE_TC_IDX );
...
void UtcDaliScriptingCreatePropertyMap()
{
  TestApplication application;
  ...
  // Non properties
  {
    ...
    DALI_TEST_CHECK( !map.empty() );
    Property::Value value( map );
    DALI_TEST_CHECK( value.HasKey( "opacity" ) );
    DALI_TEST_EQUALS( value.GetValue( "opacity" ).Get< float >(), 0.5f, TEST_LOCATION );
