...
inline void SetScissorTest( const RenderList& renderList, Context& context )
{
  ...
  if( renderList.IsClipping() )
  {
    ...
    //todor
    //clip.Set( 20, 20, 30, 30 );
    //Dali::ClippingBox clip( -800, -800, 300, 300 );
...
static int g_debt1 = 0;
#if 1
inline void SetupStencilBuffer( const RenderItem& item, Context& context, int index, bool& hitClippingRenderer, int& currentClippingDepth )
{
#if 0
  // DISABLE tmp stencil code
#if 1
  ...
  int tmpClippingId = node.GetClippingId();
  const Dali::Renderer::ClippingMode tmpClippingMode = node.GetClippingMode();
  const int tmpClippingDepth = node.GetClippingDepth();
  std::cout << "todor: ============================================== RENDERING: idx:" << index << " Node:" << node.GetName() << " Mode:" << tmpClippingMode << " Id:" << tmpClippingId << " Depth:" << tmpClippingDepth << std::endl;
...
// ==========================================================================================================================
  ...
  int stencilDepthC = tmpClippingDepth;
  int stencilDepthN = stencilDepthC;
  int eraseStencilCount = 0;
  ...
  bool stencilBufferEnabled = false;
  Dali::Renderer::ClippingMode clippingMode = tmpClippingMode;
  int clippingId = tmpClippingId;

  // Note: If we are enabling the stencil buffer to clip a renderer against, the stencil buffer
  // will have already been cleared and setup for this renderers clippingId before now.
  // TODO: optimise "stencilBufferEnabled" variable out.
  // TODO: Make clippingId -1 the invalid clipping Id.
  if( clippingId > 0 )
  {
    stencilBufferEnabled = true;
  }
  ...
  //todor
#if 1
  context.ColorMask( clippingMode != Dali::Renderer::CLIPPING_ENABLED );
#else
  context.ColorMask( true );
#endif
  ...
  if( stencilBufferEnabled )
  {
    ...
    if( clippingMode != Dali::Renderer::CLIPPING_DISABLED )
    {
      //todor optimise
      if( tmpClippingDepth < currentClippingDepth )
      {
        eraseStencilCount = 8 - tmpClippingDepth;
      }
      currentClippingDepth = tmpClippingDepth;

      // If clipping Id is 1, this is the first clipping renderer within this render-list.
      // TODO: May have to make this persistent if clipping renderers get reordered.
      if( clippingId == 1 )
      {
        // We are enabling the stencil-buffer for the first time within this render list.
        // Clear the buffer at this point.
      ...
      {
        std::cout << "todor: eraseStencilCount: " << eraseStencilCount << std::endl;
        // TODO: This variable needs calculating
        // Note: If we traverse up the scene-graph to a previous stencil,
        // we need to clear some of the stencil-buffers bitplanes.
        //std::cout << "### Erasing " << eraseStencilCount << " ###" << std::endl;
#if 1
        //int stencilClearMask = ( ( 1 << ( 8 - eraseStencilCount ) ) - 1 ) ^ 0xff;
        int stencilClearMask = 0xfe;
        context.StencilMask( stencilClearMask );
        //context.StencilOp( GL_REPLACE, GL_REPLACE, GL_REPLACE );
        context.Clear( GL_STENCIL_BUFFER_BIT, Context::CHECK_CACHED_VALUES );
#endif
      ...
      if( g_debt1 == 1 )
      {
        context.StencilFunc( GL_ALWAYS, 1u, 0u );
        context.StencilMask( stencilMaskNew );
        context.StencilOp( GL_REPLACE, GL_REPLACE, GL_REPLACE );
      }
#endif
      ...
      // TODO: Combine stencilDepthN and stencilDepthC.
      int stencilMaskNew = 1 << ( stencilDepthN - 1 );
      context.StencilFunc( GL_ALWAYS, stencilMaskNew, 0u );
      std::cout << std::hex << "todor: drawing to stencil buf, mask: " << stencilMaskNew << std::endl;
      context.StencilMask( stencilMaskNew );
      context.StencilOp( GL_KEEP, GL_REPLACE, GL_REPLACE );
    }
    else
    {
      int drawMaskCurrent = ( 1 << stencilDepthC ) - 1;
      std::cout << std::hex << "todor: reading from stencil buf, mask: " << drawMaskCurrent << std::endl;
      context.StencilFunc( GL_EQUAL, drawMaskCurrent, 0xff );
      context.StencilOp( GL_KEEP, GL_KEEP, GL_KEEP );
    }
...
{
  std::cout << "todor: ProcessRenderList:" << renderList.GetName() << std::endl;
  DALI_PRINT_RENDER_LIST( renderList );

  //SetScissorTest( renderList, context );//todor need?
  //SetRenderFlags2( renderList, context );
  SetRenderFlags( renderList, context );

  //RenderList tList = renderList;
#if 0
  unsigned int tFlags = renderList.GetFlags();
  bool hitClippingRenderer = false;
#endif
#ifdef ENABLED_NEW_CLIPPING
  int currentClippingDepth = 0;
#endif
  ...
  if( renderList.HasColorRenderItems() )
  {
    ...
    //std::cout << "todor: ProcessRenderList:     HasColorRenderItems: " << ((int)count) << std::endl;

    //bool hasHitStencil = false;
    ...
    for ( size_t index = 0; index < count; ++index )
    {
...
#if 0
      bool modifyFlags = ( index == 2 ) || ( index == 4 ) || ( index == 7 ) || ( index == 9 );

      if( modifyFlags )
      {
        hasHitStencil = true;
        //tList.SetFlags( (unsigned int)( tFlags | RenderList::STENCIL_BUFFER_ENABLED | RenderList::STENCIL_WRITE ) );
        //SetRenderFlagsTEST( tList, context );
        SetRenderFlagsTEST( (unsigned int)( tFlags | RenderList::STENCIL_BUFFER_ENABLED | RenderList::STENCIL_WRITE ), context, index );
      }
      else
      {
        if( hasHitStencil )
        {
          SetRenderFlagsTEST( (unsigned int)tFlags | RenderList::STENCIL_BUFFER_ENABLED, context, index );
        }
        else
        {
          SetRenderFlagsTEST( (unsigned int)tFlags, context, index );
        }
      }
#endif

#if 0
      std::cout << "todor: ProcessRenderList: Rendering Renderer:" << item.GetName() <<
          "  Clipping:" << (int)item.GetClippingMode() << "  ClippingId:" << item.GetClippingId() << std::endl;
#endif

      //( clippingRenderer, clippingId, )
      //SetRenderFlagsTEST2( (unsigned int)tFlags, context, index, true );
      // Note: tFlags currently unused
      //SetRenderFlagsStencilBuffer( (unsigned int)tFlags, context, index, hitClippingRenderer );
#ifdef ENABLED_NEW_CLIPPING
      bool tmpbool = true;
      //SetupStencilBuffer( item, context, index, tmpbool );
      SetupStencilBuffer( item, context, index, tmpbool, currentClippingDepth );
#endif
      ...
      //SetRenderFlagsTEST2( (unsigned int)tFlags, context, index, false );

#if 0
      if( modifyFlags )
      {
        //tList.SetFlags( (unsigned int)tFlags );
        //SetRenderFlagsTEST( (unsigned int)tFlags, context, index );
        //SetRenderFlagsTEST( (unsigned int)tFlags | RenderList::STENCIL_BUFFER_ENABLED, context, index );
        if( hasHitStencil )
        {
  ...
  {
    size_t count = renderList.Count();
    std::cout << "todor: ProcessRenderList: NOT HasColorRenderItems: " << (int)count << std::endl;
    for ( size_t index = 0; index < count; ++index )
    {
      const RenderItem& item = renderList.GetItem( index );
      DALI_PRINT_RENDER_ITEM( item );
