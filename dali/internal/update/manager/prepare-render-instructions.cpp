                               ...
                               bool tryReuseRenderList )
{
  ...
  if( !layer.IsDepthTestDisabled())
  {
    flags |= RenderList::DEPTH_BUFFER_ENABLED;
    flags |= RenderList::DEPTH_CLEAR;           //@Todo:FERRAN Not need to clear if there is only one item in the renderlist, is opaque and doesnt need depth tests
  }
