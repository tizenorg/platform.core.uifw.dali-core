...
public:
  ...
  void SetTransform( TxId tx, const Vector3& position, const Vector3& scale,  const Quaternion& orientation )
  {
    unsigned int index = mId[tx];
    mTxComponent[index].mDirty = true;
  ...
  void Update()
  {
    ...
    {
      if( mTxComponent[i].mDirty )
      {
        mLocal[i].SetTransformComponents( mTxComponent[i].mScale,mTxComponent[i].mOrientation, mTxComponent[i].mPosition + mTxComponent[i].mParentOrigin );
        mTxComponent[i].mDirty = false;
      }

      if( mParent[i] != INVALID_ID )
      {
        Matrix::Multiply( mWorld[i], mLocal[i], mWorld[ mId[mParent[i] ] ]);
      }
      else
      {
        mWorld[i] = mLocal[i];
