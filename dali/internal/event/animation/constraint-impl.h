...
private:
  ...
  void ConnectConstraint()
  {
    ...
    if ( func )
    {
      ...
      TransformManagerProperty* transformManagerProperty = dynamic_cast <TransformManagerProperty* >( targetProperty );
      if( transformManagerProperty )  //It is a property managed by the transform manager
      {
    ...
    if ( func )
    {
      ...
      else
      {
        ...
        else if ( PropertyTypes::Get< Vector3 >() == targetProperty->GetType() )
        {
          ...
          const SceneGraph::TransformManagerPropertyHandler<Vector3>* transformManagerProperty = dynamic_cast <const SceneGraph::TransformManagerPropertyHandler<Vector3>* >( targetProperty );
          if( transformManagerProperty )
          {
            if ( 0 == componentIndex )
            {
