...
int UtcDaliTextGetTextArray(void)
{
  ...
  const Integration::TextArray& textArray1 = text.GetImplementation().GetTextArray();

  DALI_TEST_EQUALS( textArray1.Count(), 11u, TEST_LOCATION );
  ...
  const Integration::TextArray& textArray2 = Internal::GetTextArray( text );

  DALI_TEST_EQUALS( textArray2.Count(), 11u, TEST_LOCATION );
  ...
  const Integration::TextArray& textArray3 = Internal::GetTextArray( Text() );

  DALI_TEST_CHECK( 0u == textArray3.Count() );
