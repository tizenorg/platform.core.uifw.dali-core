 ...
 * @param Whether frustum culling is enabled or not
 */
                                     ...
                                     bool cull )
{
  ...
  if ( cull && renderable.mRenderer->GetMaterial().GetShader()->GeometryHintEnabled( Dali::ShaderEffect::HINT_DOESNT_MODIFY_GEOMETRY ) )
  {
    ...
    const Vector3& extents = renderable.mNode->GetSize( updateBufferIndex ) * scale * 0.5f;

    inside = cameraAttachment.CheckAABBInFrustum( updateBufferIndex, position, extents );
  }
