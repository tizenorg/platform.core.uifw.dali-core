...
void Animation::Update(BufferIndex bufferIndex, float elapsedSeconds, bool& looped, bool& finished )
{
  ...
  if( mLoopCount < 0 )
  {
    ...
    // we're not incrementing mPlayCount as event loop takes this incrementing
    // to be a sign of the animation completing
  }
  else if( mCurrentLoop < mLoopCount - 1) // '-1<' here so last loop iteration uses play once below
  {
    ...
    {
      ++mCurrentLoop;
      // we're not incrementing mPlayCount until the finshed final loop
    }
  }
  else
  {
