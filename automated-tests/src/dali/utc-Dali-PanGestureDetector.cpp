...
int UtcDaliPanGestureAngleHandling(void)
{
  ...
  PanGestureDetector detector = PanGestureDetector::New();
  DALI_TEST_EQUALS( (unsigned int)detector.GetAngleCount(), 0u, TEST_LOCATION );

  detector.AddAngle( PanGestureDetector::DIRECTION_LEFT, Radian( Math::PI * 0.25 ) );
  DALI_TEST_EQUALS( (unsigned int)detector.GetAngleCount(), 1u, TEST_LOCATION );
  bool found = false;
  ...
  DALI_TEST_EQUALS( (unsigned int)detector.GetAngleCount(), 2u, TEST_LOCATION );
...
int UtcDaliPanGestureGetAngle(void)
{
  ...
  DALI_TEST_EQUALS( (unsigned int)detector.GetAngleCount(), 3u, TEST_LOCATION );

  detector.AddAngle( PanGestureDetector::DIRECTION_DOWN );
  DALI_TEST_EQUALS( (unsigned int)detector.GetAngleCount(), 4u, TEST_LOCATION );
...
int UtcDaliPanGesturePropertyIndices(void)
{
  ...
  DALI_TEST_EQUALS( (unsigned int)indices.Size(), detector.GetPropertyCount(), TEST_LOCATION );
  END_TEST;
}
