...
public:
  ...
  float ApplyAlphaFunction( float progress )
  {
    ...
    else if( mAlphaFunction.GetBezierControlPoints() != Dali::Vector4::ZERO )
    {
      if( progress < Math::MACHINE_EPSILON_1 )
      {
        result = 0.0f;
      }
      else if( (1.0f - progress) < Math::MACHINE_EPSILON_1)
      {
        result = 1.0f;
      }
      else
      {
        ...
        {
          progress > currentX ? lowerBound = currentT : upperBound = currentT;
    ...
    {
      switch(mAlphaFunction.GetPredefinedFunction())
      {
        ...
        case AlphaFunction::EASE_IN_SINE_33:
        {
          float tmp = cosf(Math::PI_2 * 33.0f / 90.0f);
          result = -1.0f * (cosf(progress * Math::PI_2 * 33.0f / 90.0f) - tmp) / (1.0f - tmp) + 1.0f;
          break;
        }
        case AlphaFunction::EASE_OUT_SINE_33:
        {
          float tmp = cosf(Math::PI_2 * 33.0f / 90.0f);
          result = (cosf((1.0f - progress) * Math::PI_2 * 33.0f / 90.0f) - tmp) / (1.0f - tmp);
          break;
        }
