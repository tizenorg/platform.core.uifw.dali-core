...
struct AdamProf
{
  AdamProf( const char* sztag, bool doprint = true ) :
    delta(0.0),
    doPrint( doprint )
  {
    if( doPrint )
  ...
  void stop()
  {
    ...
    double start = startTime.tv_sec + (double)startTime.tv_nsec * 0.000000001;
    double end = endTime.tv_sec + (double)endTime.tv_nsec * 0.000000001;
    if( doPrint )
    {
      printf("[ADAM] %s: END time: %f\n", tag, (end-start));
      fflush(stdout);
    }
    delta = (end-start);
    stopped = true;
...
struct Impl
{
  ...
  Vector<uint32_t>      mDiscardList[2];  ///< List of CRCs to be discarded
...
struct Impl2
{
  ...
  std::vector<Render::Geometry*>   mGeometries;

  int GetBatchKeyIndex( const BatchKey& key )
  {
    for( unsigned int i = 0; i < mBatchKeys.size(); ++i )
    {
      if( mBatchKeys[i].mCRC == key.mCRC )
      {
        return i;
      }
    }
...
{
  AdamProf prof("GenerateBatchMarkers");
...
{
  AdamProf prof("TransformItemVertices");
  // set input buffers
...
{
  if( !mImpl2->mBatchParents.empty() )
  {

    std::vector<BatchParent>::iterator iter = mImpl2->mBatchParents.begin();
    std::vector<BatchParent>::iterator end = mImpl2->mBatchParents.end();

    AdamProf prof("Test", true );
    ...
    for( ; iter != end; ++iter )
    {
      ...
      size_t size = batchParentNode->mBatchedChildren.Size();
      ...
      for( size_t i = 0; i < size; ++i )
      {
        ...
        Node* node = batchParentNode->mBatchedChildren[i];
        ...
        if( oldKey.mCRC != key.mCRC )
        {
          ...
          batchKeyIndex = mImpl2->GetBatchKeyIndex( key );
          if( batchKeyIndex < 0 )
          {
            batchKeyIndex = mImpl2->mBatchKeys.size();
            mImpl2->mBatchKeys.push_back( key );
            mImpl2->mGeometries.push_back( new Render::Geometry() );
            mImpl2->mGeometries[ batchKeyIndex ]->AddPropertyBuffer( batchVertexBuffer, lastElementIndex );

            lastElementIndex = currentElementIndex;
            mImpl->mUpdateManager->AddGeometry( mImpl2->mGeometries[ batchKeyIndex ] );
          }
          else
          {
            if( !mImpl2->mBatchKeys[ batchKeyIndex ].mDirty )
              continue;
          }
        }

        // TODO: not use batch info, rewrite cloning
        if( !info.mVertexFormat )
        {
          CloneVertexFormat( geometry, info );
        }
        ...
        // vertex count
        const unsigned int sourceVertexCount = vertexBufferSize / info.mVertexComponentSize;
        ...
        transformParameters.mComponentSize = info.mVertexComponentSize;
      ...
      // create renderables
      //puts("Created geometry");
      if( !info.mGeometry )
      {
        mImpl->mUpdateManager->AddPropertyBuffer( batchVertexBuffer );
        batchVertexBuffer->SetFormat( info.mVertexFormat );
        batchVertexBuffer->SetData( &vertexBufferDest, vertexBufferDest.Size() );
      }
      mImpl2->mBatchKeys[ batchKeyIndex ].mDirty = false;
      //printf("size: %d\n", (int)size);
...
{
  BatchParent batchParent = { node, true };
  mImpl2->mBatchParents.push_back( batchParent );
}

void GeometryBatcher::ResetBatchParents()
{
  mImpl2->mBatchParents.clear();
}
...
{
  return mImpl2->mBatchKeys[ batchIndex ].mRendered;
}

void GeometryBatcher::SetRendered( int batchIndex )
{
  mImpl2->mBatchKeys[ batchIndex ].mRendered = true;
}

int GeometryBatcher::GetBatchIndexFromNode( Node* node )
{
  BatchKey key( node );
  return mImpl2->GetBatchKeyIndex( key );
}

Render::Geometry* GeometryBatcher::GetGeometry( int batchIndex )
{
  return mImpl2->mGeometries[ batchIndex ];
}

void GeometryBatcher::PrepareToRender()
{
  for( size_t i = 0; i < mImpl2->mBatchKeys.size(); ++i )
  {
    BatchKey& key = mImpl2->mBatchKeys[i];
    key.mRendered = false;
  }
}
