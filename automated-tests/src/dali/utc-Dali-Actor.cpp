...
int UtcDaliActorSetOpacity(void)
{
  ...
  actor.OpacityTo( actor.GetCurrentOpacity() + 0.1f );
  // flush the queue and render once
  application.SendNotification();
  application.Render();
  ...
  // change opacity, actor is on stage to change is not immediate
  actor.OpacityBy( actor.GetCurrentOpacity() -0.9f );
  // flush the queue and render once
  application.SendNotification();
  application.Render();
...
int UtcDaliActorSetColor(void)
{
  ...
  actor.ColorTo( actor.GetCurrentColor() - Vector4( -0.4f, -0.5f, -0.6f, -0.4f ) );
  // flush the queue and render once
  application.SendNotification();
  application.Render();
  ...
  actor.ColorTo( actor.GetCurrentColor() - Vector4( 1.1f, 1.1f, 1.1f, 1.1f ) );
  // flush the queue and render once
  application.SendNotification();
  application.Render();
...
const PropertyStringIndex PROPERTY_TABLE[] =
{
  ...
  { "rotation",                 Actor::Property::ORIENTATION,              Property::ROTATION    },
  { "world-rotation",           Actor::Property::WORLD_ORIENTATION,        Property::ROTATION    },
  ...
  { "inherit-rotation",         Actor::Property::INHERIT_ORIENTATION,      Property::BOOLEAN     },
