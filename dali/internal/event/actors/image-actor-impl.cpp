...
ImageActorPtr ImageActor::New()
{
  ...
  actor->mRenderer->SetTextureSet( *textureSet );
...
void ImageActor::SetImage( ImagePtr& image )
{
  ...
  else
  {
    ...
    mRenderer->GetTextureSet()->SetImage( 0u, image.Get() );
    mRenderer->GetTextureSet()->SetSampler( 0u, sampler );
...
{
  return mRenderer->GetTextureSet()->GetImage( 0u );
}
...
void ImageActor::UpdateTexureRect()
{
  ...
  TextureSet* textureSet = mRenderer->GetTextureSet();
...
void ImageActor::SetFilterMode( FilterMode::Type minFilter, FilterMode::Type magFilter )
{
  ...
  mRenderer->GetTextureSet()->SetSampler( 0, sampler.Get() );
}
...
void ImageActor::EffectImageUpdated()
{
  if( mShaderEffect )
  {
    ...
    if( effectImage )
    {
      ...
      mRenderer->GetTextureSet()->SetImage( 1u, &effectImageImpl );
    }
    else
    {
       mRenderer->GetTextureSet()->SetImage( 1u, 0 );
    }
  }
}
