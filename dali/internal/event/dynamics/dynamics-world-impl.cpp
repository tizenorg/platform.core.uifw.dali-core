...
Dali::Integration::DynamicsFactory* Dali::Internal::DynamicsWorld::mDynamicsFactoryInstance( NULL );
...
{
  if( !mDynamicsFactoryInstance )
  {
    mDynamicsFactoryInstance = ThreadLocalStorage::Get().GetPlatformAbstraction().GetDynamicsFactory();
  }

  if( mDynamicsFactoryInstance && !mDynamicsWorldInstance )
  {
    if( mDynamicsFactoryInstance->InitializeDynamics( *( configuration->GetSettings() ) ) )
    {
      StagePtr stage = Stage::GetCurrent();
      if( stage != NULL )
      {
        mDynamicsWorldInstance = DynamicsWorld::New();
        mDynamicsWorldInstance->Initialize( *stage, *mDynamicsFactoryInstance, configuration );
      }
    }
  }
