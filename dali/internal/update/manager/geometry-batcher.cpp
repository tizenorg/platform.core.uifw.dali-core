...
void TransformVertexBuffer( TransformVertexBufferData& data )
{
  ...
  Dali::Internal::MultiplyMatrices( transform, data.worldMatrix, data.parentInvWorldMatrix );
  ...
  if( !mImpl->batchParents.empty() )
  {
    ...
    for( size_t batchParentIndex = 0; iter != end; ++iter, ++batchParentIndex )
    {
      ...
      for( size_t i = 0; i < size; ++i )
      {
        ...
        transformParameters.worldMatrix = worldMatrix.AsFloat();
        transformParameters.parentInvWorldMatrix = invWorldMatrix.AsFloat();
