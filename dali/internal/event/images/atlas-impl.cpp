                    ...
                    std::size_t yOffset)
{
  ...
  if( bitmap && Compatible(bitmap->GetPixelFormat(),
                           xOffset + bitmap->GetImageWidth(),
                           yOffset + bitmap->GetImageHeight()) )
  {
    AllocateAtlas();
    ResourceId destId = GetResourceId();
    if( destId )
    {
      mResourceClient.UploadBitmap( destId, bitmap.Get(), xOffset, yOffset );
