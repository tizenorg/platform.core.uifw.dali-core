...
bool CameraAttachment::CheckAABBInFrustum( BufferIndex bufferIndex, const Vector3& origin, const Vector3& extents )
{
  const FrustumPlanes& planes = mFrustum[ bufferIndex ];

  for ( uint32_t i = 0; i < 6; ++i )
  {
    Vector3 signFlip(Sign(planes.mPlanes[ i ].mNormal.x), Sign(planes.mPlanes[ i ].mNormal.y), Sign(planes.mPlanes[ i ].mNormal.z));
    if( planes.mPlanes[ i ].mNormal.Dot( origin + (extents * signFlip) ) > -(planes.mPlanes[ i ].mDistance) )
    {
      continue;
    }
