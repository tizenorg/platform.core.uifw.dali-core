...
public:
  ...
  enum PredefinedFunction
  {
    ...
    EASE_IN_SINE_33,    ///< Speeds up and comes to a sudden stop (sinusoidal)
    EASE_OUT_SINE_33,   ///< Sudden start and slows to a gradual stop (sinusoidal)
  ...
  enum Mode
  {
    PREDEFINED_FUNCTION,  //< The user has specified a predefined function
   ...
   * Creates an alpha function object with the default predefined alpha function
  ...
  /**
   * @brief Constructor.
   * Creates an alpha function object with the predefined alpha function passed as a parameter
   * to the constructor
   * @param[in] function One of the predefined alpha functions
   * @return The alpha function
   */
  AlphaFunction( PredefinedFunction function);
   ...
   * @param[in] cp0 A Vector2 which will be used as the first control point of the curve
   * @param[in] cp1 A Vector2 which will be used as the second control point of the curve
   * @return The alpha function
   */
  AlphaFunction( const Dali::Vector2& cp0, const Dali::Vector2& cp1 );
  ...
  /**
   * @brief Returns the predefined function used by the alpha function
   * @return One of the predefined alpha functions. In case no predefined function
   * has been specified, it will return AlphaFunction::DEfAULT
   */
  PredefinedFunction GetPredefinedFunction() const;
...
private:
  ...
  PredefinedFunction      mPredefined;            //< Enum indicating the predefined alpha function
