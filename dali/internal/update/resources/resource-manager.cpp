...
void ResourceManager::HandleLoadShaderRequest( ResourceId id, const ResourceTypePath& typePath )
{
  ...
  if( shaderType )
  {
    ...
    const bool loadResult = mImpl->mPlatformAbstraction.LoadShaderBinaryFile( typePath.path, shaderData->GetBuffer() );
    DALI_LOG_INFO( Debug::Filter::gResource, Debug::General, "Loaded %s with result %s.\n", typePath.path.c_str(), loadResult ? "true" : "false" );
