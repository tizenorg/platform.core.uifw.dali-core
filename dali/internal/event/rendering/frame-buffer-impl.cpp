...
void FrameBuffer::AttachColorTexture( NewTexturePtr texture, unsigned int mipmapLevel, CubeMap::Face face )
{
  ...
  {
    mColor = texture;
    AttachColorTextureToFrameBuffer( mEventThreadServices.GetUpdateManager(), *mRenderObject, texture->GetRenderObject(), mipmapLevel, static_cast<unsigned int>(face) );
  }
}
