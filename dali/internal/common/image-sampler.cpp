 ...
 */
void StoreFilterMode( unsigned int& options, unsigned int mode, int bitShift )
{
  if( mode != 0 )
  {
 ...
 */
unsigned int RetrieveField( unsigned int options, int mask, int bitShift )
{
...
unsigned int PackBitfield( FilterMode::Type minify, FilterMode::Type magnify, WrapMode::Type uWrap, WrapMode::Type vWrap )
{
  ...
  StoreFilterMode( bitfield, uWrap, UWRAP_BIT_SHIFT );
  StoreFilterMode( bitfield, vWrap, VWRAP_BIT_SHIFT );
  return bitfield;
}

FilterMode::Type GetMinifyFilterMode( unsigned int bitfield )
{
  return (FilterMode::Type)RetrieveField( bitfield, MASK_MINIFY_FILTER, MINIFY_BIT_SHIFT );
}

FilterMode::Type GetMagnifyFilterMode( unsigned int bitfield )
{
  return (FilterMode::Type)RetrieveField( bitfield, MASK_MAGNIFY_FILTER, MAGNIFY_BIT_SHIFT );
}

WrapMode::Type GetUWrapMode( unsigned int bitfield )
{
  return (WrapMode::Type)RetrieveField( bitfield, MASK_UWRAP_MODE, UWRAP_BIT_SHIFT );
}

WrapMode::Type GetVWrapMode( unsigned int bitfield )
{
  return (WrapMode::Type)RetrieveField( bitfield, MASK_VWRAP_MODE, VWRAP_BIT_SHIFT );
}
