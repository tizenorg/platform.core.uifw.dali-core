...
int utcDaliPathGetPointCount(void)
{
  ...
  DALI_TEST_EQUALS(path.GetPointCount(), (size_t)0u, TEST_LOCATION);
  ...
  DALI_TEST_EQUALS(path.GetPointCount(), (size_t)4u, TEST_LOCATION);

  path.AddPoint(Vector3(330.0, 220.0, 0.0));
  path.AddPoint(Vector3(400.0,  50.0, 0.0));

  DALI_TEST_EQUALS(path.GetPointCount(), (size_t)6u, TEST_LOCATION);
  END_TEST;
}
