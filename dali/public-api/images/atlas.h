 ...
 * By default, the atlas is responsible for re-upload the resource images automatically,
 * while the buffer images are left to the client to upload again by connecting to the Stage::ContextRegainedSignal().
 * However with the resource image overlapping the buffer image in the atlas, the recovered context might be wrong.
