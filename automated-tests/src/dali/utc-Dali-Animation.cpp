...
int UtcDaliAnimationSetDirection(void)
{
  TestApplication application;
  ...
  // Build the animation
    float durationSeconds(1.0f);
    Animation animation = Animation::New(durationSeconds);
    animation.SetPlayDirection(Dali::Animation::PlayDirectionBackward);
    ...
    animation.SetPlayDirection(Dali::Animation::PlayDirectionForward);
    animation.Play(); // Test that calling play has no effect, when animation is already playing
    application.SendNotification();
    application.Render(static_cast<unsigned int>(durationSeconds*200.0f)/* 60% progress */);
...
int UtcDaliAnimationGetDirection(void)
{
  TestApplication application;

  Animation animation = Animation::New(1.0f);
  DALI_TEST_CHECK(animation.GetPlayDirection() == Dali::Animation::PlayDirectionForward);

  animation.SetPlayDirection(Dali::Animation::PlayDirectionBackward);
  DALI_TEST_CHECK(animation.GetPlayDirection() == Dali::Animation::PlayDirectionBackward);
  END_TEST;
}
