...
void RenderGeometry::SetIndexBuffer( Dali::Vector<unsigned short>* indices )
{
  mIndices = indices;
    ...
    Vector<GLint>& attributeLocation )
{
  if( !mHasBeenUpdated )
  {
    ...
    if( mIndicesChanged )
    {
      ...
      else
      {
        ...
        std::size_t dataSize =  sizeof( unsigned short ) * mIndices.Get()->Size();
        DALI_ASSERT_DEBUG( dataSize != 0u && "No data in the index buffer!" );
        const GLvoid* data = &((*mIndices)[0]);
        mIndexBuffer->UpdateDataBuffer( dataSize, data, GpuBuffer::STATIC_DRAW, GpuBuffer::ELEMENT_ARRAY_BUFFER );
      }
  ...
  if( mIndices && geometryGLType != GL_POINTS)
  {
    //Indexed draw call
    numIndices = mIndices->Size();
    mIndexBuffer->Bind( GpuBuffer::ELEMENT_ARRAY_BUFFER );
    context.DrawElements(geometryGLType, numIndices, GL_UNSIGNED_SHORT, 0);
  }
  else
  {
    //Unindex draw call
