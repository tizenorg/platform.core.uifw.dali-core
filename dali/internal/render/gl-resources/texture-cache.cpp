...
void TextureCache::Clear( ResourceId id, const Vector4& color )
{
  DALI_LOG_INFO(Debug::Filter::gGLResource, Debug::General, "TextureCache::Clear(id: %d)\n", id);
  TextureIter textureIter = mTextures.find(id);
  DALI_ASSERT_DEBUG( textureIter != mTextures.end() );
  if( textureIter != mTextures.end() )
  {
...
void TextureCache::DispatchUpdateTexture( ResourceId id, Bitmap* bitmap , std::size_t xOffset, std::size_t yOffset)
{
  // NULL, means being shutdown, so ignore msgs
  if( mSceneGraphBuffers != NULL )
  {
...
void TextureCache::DispatchClear( ResourceId id, const Vector4& color )
{
  // NULL, means being shutdown, so ignore msgs
  if( mSceneGraphBuffers != NULL )
  {
    typedef MessageValue2< TextureCache, ResourceId, Vector4 > DerivedType;

    // Reserve some memory inside the render queue
    unsigned int* slot = mRenderQueue.ReserveMessageSlot( mSceneGraphBuffers->GetUpdateBufferIndex(), sizeof( DerivedType ) );

    // Construct message in the render queue memory; note that delete should not be called on the return value
    new (slot) DerivedType( this, &TextureCache::Clear, id, color );
  }
}
