  ...
  bool cullMode )
{
  ...
  if( renderList.HasColorRenderItems() )
  {
    ...
    for ( size_t index = 0; index < count; ++index )
    {
      ...
      Render::Renderer* renderer = const_cast< Render::Renderer* >( item.GetRenderer() );
      renderer->Render( context, textureCache, bufferIndex, item.GetNode(), defaultShader, item.GetModelViewMatrix(), viewMatrix, projectionMatrix, frameTime, cullMode, !item.IsOpaque() );
    }
  }
  else
  {
  ...
  size_t&                   currentIndex ) // Out parameter
{
  ...
  for( ; currentIndex < count ; currentIndex++ )
  {
    ...
    {
      Render::Renderer* renderer = const_cast< Render::Renderer* >( renderItem.GetRenderer() );
      const Matrix& modelViewMatrix = renderItem.GetModelViewMatrix();
      renderer->Render( context, textureCache, bufferIndex, renderItem.GetNode(), defaultShader, modelViewMatrix, viewMatrix, projectionMatrix, frameTime, cullMode, !renderItem.IsOpaque() );
