...
void TextAttachment::SetTextColor( const Vector4& color )
{
  ...
  if( sendMessage )
  {
    SetTextColorMessage( mEventThreadServices, *mSceneObject, clampedColor );
  }
}
...
void TextAttachment::ResetTextColor()
{
  if( NULL != mTextColor )
  {
    ...
    SetTextColorMessage( mEventThreadServices, *mSceneObject, TextStyle::DEFAULT_TEXT_COLOR );
  }
}
...
void TextAttachment::SetOutline( bool enable, const Vector4& color, const Vector2& thickness )
{
      ...
      ( mStyle.GetOutlineThickness() != thickness ) )
  {
    ...
    SetOutlineMessage( mEventThreadServices, *mSceneObject, enable, color, thickness );
  }
}
...
void TextAttachment::ResetOutline()
{
  if( !mStyle.IsOutlineDefault() )
  {
    ...
    SetOutlineMessage( mEventThreadServices, *mSceneObject, false, TextStyle::DEFAULT_OUTLINE_COLOR, TextStyle::DEFAULT_OUTLINE_THICKNESS );
  }
}
...
void TextAttachment::SetGlow( bool enable, const Vector4& color, float intensity )
{
      ...
      ( fabsf( mStyle.GetGlowIntensity() - intensity ) > Math::MACHINE_EPSILON_1000 ) )
  {
    ...
    SetGlowMessage( mEventThreadServices, *mSceneObject, enable, color, intensity );
  }
}
...
void TextAttachment::ResetGlow()
{
  if( !mStyle.IsGlowDefault() )
  {
    ...
    SetGlowMessage( mEventThreadServices, *mSceneObject, false, TextStyle::DEFAULT_GLOW_COLOR, TextStyle::DEFAULT_GLOW_INTENSITY );
  }
}
...
void TextAttachment::SetShadow( bool enable, const Vector4& color, const Vector2& offset, float size )
{
      ...
      ( fabsf( mStyle.GetShadowSize() - size ) > Math::MACHINE_EPSILON_1000 ) )
  {
    ...
    SetDropShadowMessage( mEventThreadServices, *mSceneObject, enable, color, shadowOffset, shadowSize );
  }
}
...
void TextAttachment::ResetShadow()
{
  if( !mStyle.IsShadowDefault() )
  {
    ...
    shadowOffset = Max( shadowOffset, -maxOffset );
    shadowOffset *= unitPointSize / fontPointSize;
    SetDropShadowMessage( mEventThreadServices, *mSceneObject, false, TextStyle::DEFAULT_SHADOW_COLOR, shadowOffset, shadowSize );
  }
}
...
void TextAttachment::SetGradient( const Vector4& color, const Vector2& startPoint, const Vector2& endPoint )
{
      ...
      ( mStyle.GetGradientEndPoint() != endPoint ) )
  {
    ...
    SetGradientMessage( mEventThreadServices, *mSceneObject, color, startPoint, endPoint );
  }
}
...
void TextAttachment::ResetGradient()
{
  if( !mStyle.IsGradientDefault() )
  {
    ...
    SetGradientMessage( mEventThreadServices, *mSceneObject, TextStyle::DEFAULT_GRADIENT_COLOR, TextStyle::DEFAULT_GRADIENT_START_POINT, TextStyle::DEFAULT_GRADIENT_END_POINT );
  }
}
...
void TextAttachment::CalculateWeightedSmoothing( TextStyle::Weight weight, float smoothEdge )
{
  ...
  SetSmoothEdgeMessage( mEventThreadServices, *mSceneObject, weightedSmoothing );
}
...
void TextAttachment::SetTextChanges()
{
  if( mVertexBuffer )
  {
    ...
    EventThreadServices& eventThreadServices(  mEventThreadServices );
    ...
    if( mTextChanged  || mFontChanged )
    {
      ...
      SetTextVertexBufferMessage( eventThreadServices, attachment, *mVertexBuffer.Release() );

      if( mFontChanged )
      {
        SetTextFontSizeMessage( eventThreadServices, attachment, mFont->GetPixelSize() );
      }
    }
  }
}
