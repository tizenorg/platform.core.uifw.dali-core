...
int UtcDaliRenderTaskWorldToScreen(void)
{
  ...
  Vector3 position = actor.GetCurrentWorldPosition();
  float screenX = 0.0;
  float screenY = 0.0;

  bool ok = task.WorldToScreen(actor.GetCurrentWorldPosition(), screenX, screenY);
  ...
  ok = task.WorldToScreen(actor.GetCurrentWorldPosition(), screenX, screenY);
  DALI_TEST_CHECK(ok == true);
...
int UtcDaliRenderTaskScreenToLocal(void)
{
  TestApplication application;
  Actor actor = Actor::New();
  ...
  DALI_TEST_CHECK( task.ScreenToLocal(actor, screenX, screenY, rtLocalX, rtLocalY ) );
