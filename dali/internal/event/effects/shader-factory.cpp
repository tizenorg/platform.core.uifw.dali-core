...
ResourceTicketPtr ShaderFactory::Load(const std::string& vertexSource, const std::string& fragmentSource, size_t& shaderHash)
{
  ...
  // add dali version to the beginning of the hash (1-0-31-)
  ...
  std::string hashString = stringHash.str();
  std::string filename;
  filename.reserve( hashString.size() + sizeof(SHADER_SUFFIX)/sizeof(char) );
  filename.append( hashString );
