...
const StringEnum< SamplingMode > IMAGE_SAMPLING_MODE_TABLE[] =
{
...
Image NewImage( const Property::Value& map )
{
  ...
  if( Property::MAP == map.GetType() )
  {
    ...
    if( map.HasKey(field) )
    {
      ...
      std::string s( value.Get<std::string>());
      attributes.SetFilterMode( GetEnumeration< SamplingMode >( s.c_str(), IMAGE_SAMPLING_MODE_TABLE, IMAGE_SAMPLING_MODE_TABLE_COUNT ) );
    }
