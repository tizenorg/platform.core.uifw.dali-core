                          ...
                          Dali::Vector< NodeRenderer >& container )
{
  ...
  else
  {
    ...
    for( unsigned int i=batchBegin; i<batchEnd; ++i )
    {
      ...
      const unsigned int size = data->Count() / sizeof(float);

      for( float* currentVertex = const_cast<float*>(vertices); currentVertex < vertices+size; currentVertex+=4 )
      {
        ...
        batchInfo->vertices.Insert( batchInfo->indices.End(), reinterpret_cast<char*>(&worldVertex.x), reinterpret_cast<char*>(&worldVertex.w) );
        batchInfo->vertices.Insert( batchInfo->indices.End(), reinterpret_cast<char*>(currentVertex+2), reinterpret_cast<char*>(currentVertex+4) );
      }
