...
private:
  ...
  FreeList mId;
  std::vector<TransformComponentAnimatable> mTxComponentAnimatable; //Animatable part of the components
  std::vector<TransformComponentStatic> mTxComponentStatic;         //Static part of the components
  std::vector<unsigned int> mInheritanceMode;                       //Inheritance mode of the components
  std::vector<TransformId> mComponentId;                            //Ids of the components
  std::vector<Vector3> mSize;                                       //Size of the components
  std::vector<TransformId> mParent;                                 //Parent of the components
  std::vector<Matrix> mWorld;                                       //Local to world transform of the components
  std::vector<Matrix> mLocal;                                       //Local to parent space transform of the components

  std::vector<TransformComponentAnimatable> mTxComponentAnimatableBaseValue;  //Base values for the animatable part of the components
  std::vector<Vector3> mSizeBase;                                             //Base value for the size of the components

  std::vector<unsigned int> mComponentDirty;    //1u if some of the parts of the component has changed in this frame, 0 otherwise
  std::vector<unsigned int> mSizeDirty;         //1u if size of the component has changed in this frame, 0 otherwise
  std::vector<unsigned int> mLocalMatrixDirty;  //1u if the local matrix has been updated in this frame, 0 otherwise
