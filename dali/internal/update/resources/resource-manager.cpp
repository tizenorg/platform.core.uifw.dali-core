...
{
  DALI_ASSERT_DEBUG( mImpl->mResourceClient != NULL );
  DALI_LOG_INFO(Debug::Filter::gResource, Debug::General, "ResourceManager: LoadComplete(id:%u, status=%s)\n", id, loadStatus==RESOURCE_LOADING?"LOADING":loadStatus==RESOURCE_PARTIALLY_LOADED?"PARTIAL":"COMPLETE");
  ...
  if (iter != mImpl->loadingRequests.end())
  {
    ...
    switch (type)
    {
      case ResourceBitmap:
      {
        ...
        UpdateImageTicket (id, attrs);
        ///!We send three values through to the client in an object with many data members and expect it to update the three values that we want it too and not update any others (like crop), with no code-level contract to enforce the behaviour we want.
      ...
      case ResourceText:
      {
        //TODO Remove - Probably don't need to send glyph set back anymore
...
void ResourceManager::LoadFailed(ResourceId id, ResourceFailure failure)
{
  ...
  if (iter != mImpl->loadingRequests.end())
  {
    ...
    // Let NotificationManager know that the resource manager needs to do some processing
    // TODO - pass failure cause onto ticket
    NotifyTickets();
  }
}
