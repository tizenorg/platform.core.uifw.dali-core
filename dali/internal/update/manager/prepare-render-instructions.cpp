                          ...
                          Dali::Vector< NodeRenderer >& container )
{
  ...
  else
  {
    ...
    //batchInfo->batchVertices.Reserve( (batchEnd - batchBegin) * 16 * sizeof(Vertex2D) );
    //batchInfo->batchIndices.Reserve( (batchEnd - batchBegin) * 24 * sizeof(?) );
    ...
    for( unsigned int i=batchBegin; i<batchEnd; ++i )
    {
      ...
      if( vertices )
      {
        batchInfo->vertices.Insert( batchInfo->vertices.End(), vertices->Begin(), vertices->End() );
      }
    ...
    // FIXME - Remove this
    const Matrix& worldMatrix = container[ batchBegin ].mNode->GetWorldMatrix( updateBufferIndex );
    Matrix::Multiply( item.GetModelViewMatrix(), worldMatrix, viewMatrix );
  }
}
