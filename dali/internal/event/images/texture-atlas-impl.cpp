...
TextureAtlas::SelectedBlock TextureAtlas::FindSmallestBlock( const unsigned int width, const unsigned int height )
{
  ...
  // Check to see if this free block, has any alternates
  if ( thisBlock.alternates[ 0 ] )
  {
    printf("\tDiscarding blocks %i and %i\n", thisBlock.alternates[0] - 1u, thisBlock.alternates[1] -1u );

    // Make sure the current old alternates are removed
    unsigned int ref1 = thisBlock.alternates[ 0 ];
    unsigned int ref2 = thisBlock.alternates[ 1 ];

    if ( ref1-- )
    {
      mFreeBlocks[ ref1 ].block.w = 0u;
    }

    if ( ref2-- )
    {
      mFreeBlocks[ ref2 ].block.w = 0u;
...
void TextureAtlas::DiscardBlock( const unsigned int block )
{
  ...
  if ( thisBlock )
  {
    // Remove references from the alternates to this block
    unsigned int alt1 = mFreeBlocks[ block ].alternates[ 0 ];
    unsigned int alt2 = mFreeBlocks[ block ].alternates[ 1 ];
  ...
  printf("Ater reference removal\n");
  DisplayFreeBlocks();

  // Replace the last block in the list with this unused block
  unsigned int current = mFreeBlockCount;
  mFreeBlocks[ thisBlock ] = mFreeBlocks[ --mFreeBlockCount ];

  // Check for any alternate references that need fixing up
  unsigned int alt1 = mFreeBlocks[ block ].alternates[ 0 ];
  unsigned int alt2 = mFreeBlocks[ block ].alternates[ 1 ];

  if ( alt1-- )
  {
    if ( mFreeBlocks[ alt1 ].alternates[ 0 ] == current )
    {
      mFreeBlocks[ alt1 ].alternates[ 0 ] = block;
    }
    else
    {
      if ( mFreeBlocks[ alt1 ].alternates[ 1 ] == current )
      {
  ...
  if ( alt2-- )
  {
    if ( mFreeBlocks[ alt2 ].alternates[ 0 ] == current )
    {
      mFreeBlocks[ alt2 ].alternates[ 0 ] = block;
    }
    else
    {
      if ( mFreeBlocks[ alt2 ].alternates[ 1 ] == current )
      {
