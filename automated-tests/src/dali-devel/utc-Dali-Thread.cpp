...
int UtcDaliThreadP(void)
{
  ...
  DALI_TEST_EQUALS( Thread::STATE_NONE, thread.GetCurrentState(), TEST_LOCATION );

  thread.Start();

  DALI_TEST_EQUALS( Thread::STATE_STARTED, thread.GetCurrentState(), TEST_LOCATION );
  ...
  thread.Join();
  DALI_TEST_EQUALS( Thread::STATE_JOINED, thread.GetCurrentState(), TEST_LOCATION );
