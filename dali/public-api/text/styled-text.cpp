...
struct StyledText::Impl
{
  ...
  void GetStyleRanges( Vector<std::size_t>& positions ) const
  {
    ...
    Vector<std::size_t>::Iterator it = std::unique( positions.Begin(), positions.End() );
  ...
  void Remove( std::size_t position, std::size_t numberOfCharacters )
  {
    ...
    Vector<StyledCharacters*>::Iterator removeIt = std::remove_if( mStyles.Begin(), endIt, HasZeroCharacters );
