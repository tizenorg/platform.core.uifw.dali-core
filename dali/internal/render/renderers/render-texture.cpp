...
void NewTexture::Upload( Context& context, Vector<unsigned char>& buffer, const Dali::TextureUploadParams& params  )
{
  ...
  else if( mType == TextureType::TEXTURE_CUBE )
  {
    ...
    {
      //Specifying the whole image for the mipmap. We cannot assume that storage for that mipmap has been created so we need to use TexImage2D
      context.TexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + params.face, params.mipmap, mInternalFormat, params.width, params.height, 0, mInternalFormat, mPixelDataType, &buffer[0] );
    }
    else
    {
      //Specifying part of the image for the mipmap
      context.TexSubImage2D( GL_TEXTURE_CUBE_MAP_POSITIVE_X + params.face, params.mipmap, params.x0, params.y0, params.width, params.height, mInternalFormat, mPixelDataType, &buffer[0] );
    }
  }
}
