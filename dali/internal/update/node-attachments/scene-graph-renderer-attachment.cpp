...
void RendererAttachment::ConnectedToSceneGraph()
{
  ...
  DataProviders* dataProviders = NewDataProviders();
  mRenderer = NewRenderer::New( *mParent, dataProviders );
...
void RendererAttachment::DoPrepareRender( BufferIndex updateBufferIndex )
{
  if( mRegenerateUniformMap > 0)
  {
    if( mRegenerateUniformMap == REGENERATE_UNIFORM_MAP)
    {
      ...
      const DataProviders::Samplers& samplers = mMaterial->GetSamplers();
      for( DataProviders::Samplers::ConstIterator iter = samplers.Begin(), end = samplers.End();
           iter != end ;
           ++iter )
      {
  ...
  if( mResendDataProviders )
  {
    ...
    typedef MessageValue1< NewRenderer, OwnerPointer<DataProviders> > DerivedType;
    unsigned int* slot = mSceneController->GetRenderQueue().ReserveMessageSlot( updateBufferIndex, sizeof( DerivedType ) );
    new (slot) DerivedType( mRenderer, &NewRenderer::SetDataProviders, dataProviders );

    mResendDataProviders = false;
  }
}

DataProviders* RendererAttachment::NewDataProviders()
{
  DataProviders* dataProviders = new DataProviders();
  dataProviders->SetGeometry( mGeometry );
  dataProviders->SetVertexBuffers( mGeometry->GetVertexBuffers() );
  dataProviders->SetIndexBuffer( mGeometry->GetIndexBuffer() );
  dataProviders->SetMaterial(mMaterial);
  dataProviders->SetSamplers(mMaterial->GetSamplers());
  dataProviders->SetShader(mMaterial->GetShader());
  dataProviders->SetUniformMap(this);
...
bool RendererAttachment::IsFullyOpaque( BufferIndex updateBufferIndex )
{
  ...
  if( mMaterial != NULL )
  {
    ...
    const DataProviders::Samplers& samplers = mMaterial->GetSamplers();
    for( DataProviders::Samplers::ConstIterator iter = samplers.Begin();
