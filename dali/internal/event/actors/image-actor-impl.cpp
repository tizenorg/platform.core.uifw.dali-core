...
void ImageActor::SetImage( ImagePtr& image )
{
  ...
  if( newImage )
  {
    ...
    NPatchImage* ninePatchImage = NPatchImage::DownCast( image.Get() );
    if( ninePatchImage )
    {
      newImage = ninePatchImage->CreateCroppedBufferImage();
      SetStyle( Dali::ImageActor::STYLE_NINE_PATCH );

      const NPatchImage::StretchRanges& stretchPixelsX = ninePatchImage->GetStretchPixelsX();
      const NPatchImage::StretchRanges& stretchPixelsY = ninePatchImage->GetStretchPixelsY();
