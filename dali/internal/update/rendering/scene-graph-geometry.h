...
public:
  ...
  /**
   * Sets the center of the geometry
   * @param[in] center The center of the geometry
   */
  void SetGeometryCenter( const Vector3& centre );
...
inline void SetGeometryCenterMessage( EventThreadServices& eventThreadServices, const Geometry& geometry, const Vector3& center )
{
  typedef MessageValue1< Geometry, Vector3 > LocalType;

  // Reserve some memory inside the message queue
  unsigned int* slot = eventThreadServices.ReserveMessageSlot( sizeof( LocalType ) );

  // Construct message in the message queue memory; note that delete should not be called on the return value
  new (slot) LocalType( &geometry, &Geometry::SetGeometryCenter, center );
}
