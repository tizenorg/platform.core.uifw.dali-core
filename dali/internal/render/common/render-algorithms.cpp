...
inline void SetupPerRendererFlags( const RenderItem& item, Context& context, bool& usedStencilBuffer, bool stencilManagedByDrawMode )
{
  ...
  if( renderer->GetStencilMode() != StencilMode::ON )
  {
    ...
    DALI_LOG_ERROR( "todorstencil: Disabling stencil buffer\n" );
    return;
  }
  ...
  DALI_LOG_ERROR( "todorstencil: Enabling stencil buffer & setting: Func(%d,%d,%d)  Op(%d,%d,%d)  Mask(%d)\n",
      DaliStencilFunctionToGL[ renderer->GetStencilFunction() ], renderer->GetStencilFunctionReference(), renderer->GetStencilFunctionMask(),
      DaliStencilOperationToGL[ renderer->GetStencilOperationOnFail() ], DaliStencilOperationToGL[ renderer->GetStencilOperationOnZFail() ], DaliStencilOperationToGL[ renderer->GetStencilOperationOnZPass() ],
      renderer->GetStencilMask() );
