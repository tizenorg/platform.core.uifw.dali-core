  ...
  void CreateAnimator()
  {
    ...
    if( mComponentIndex == Property::INVALID_COMPONENT_INDEX )
    {
      ...
      if( animatableProperty == NULL )
      {
        ...
        mAnimator = SceneGraph::AnimatorTransformProperty< PropertyType,TransformManagerPropertyAccessor<PropertyType> >::New( *propertyOwner, *p, mAnimatorFunction, mAlphaFunction, mTimePeriod );
      }
      else
      {
        //Create the animator
    ...
    {
      {
        ...
        else if ( PropertyTypes::Get< Vector3 >() == baseProperty->GetType() )
        {
          ...
          if( animatableProperty == NULL )
          {
            const SceneGraph::TransformManagerPropertyHandler<Vector3>* p = dynamic_cast <const SceneGraph::TransformManagerPropertyHandler<Vector3>* >( baseProperty  );
            if( p )
            {
              if( mComponentIndex == 0 )
              {
                mAnimator = SceneGraph::AnimatorTransformProperty< float,TransformManagerPropertyComponentAccessor<Vector3,0> >::New( *propertyOwner, *p, mAnimatorFunction, mAlphaFunction, mTimePeriod );
              }
              else if( mComponentIndex == 1 )
              {
                mAnimator = SceneGraph::AnimatorTransformProperty< float,TransformManagerPropertyComponentAccessor<Vector3,1> >::New( *propertyOwner, *p, mAnimatorFunction, mAlphaFunction, mTimePeriod );
              }
              else if( mComponentIndex == 2 )
              {
                mAnimator = SceneGraph::AnimatorTransformProperty< float,TransformManagerPropertyComponentAccessor<Vector3,2> >::New( *propertyOwner, *p, mAnimatorFunction, mAlphaFunction, mTimePeriod );
              }
            }
            else
            {
