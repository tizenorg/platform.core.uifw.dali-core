...
NewTexture::NewTexture( Dali::TextureType::Type type, Pixel::Format format, unsigned int width, unsigned int height )
:mId( 0 ),
 mType( type ),
...
{
  //Creates the texture but doesn't reserve memory for it. Memory is reserved when needed in Upload method
...
void NewTexture::Upload( Context& context, Vector<unsigned char>& buffer, const Dali::TextureUploadParams& params  )
{
  if( mType == TextureType::TEXTURE_2D )
  {
    ...
    if( params.x0 == 0 && params.y0 == 0 &&
        params.width == (unsigned int)(mWidth / powf(2.0f,params.mipmap)) &&
        params.height == (unsigned int)(mHeight / powf(2.0f,params.mipmap )) )
    {
    ...
    {
      //Specifying part of the image for the mipmaps
