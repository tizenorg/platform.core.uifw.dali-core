...
: mContext(context),
  mId(0),
  mSamplerBitfield( ImageSampler::UninitialisedBitfield() ),
  mWidth(width),
  mHeight(height),
...
void Texture::ApplySampler( unsigned int samplerBitfield )
{
  if( mSamplerBitfield != samplerBitfield )
  {
    ...
    else if( ImageSampler::IsMinifyInitialized( mSamplerBitfield ) )
    {
      // Reset to system default option
      mContext.TexParameteri( GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, FilterModeToGL( ImageSampler::GetMinifyFilterMode( ImageSampler::DefaultOptions() ) ) );
    }
    ...
    else if( ImageSampler::IsMagnifyInitialized( mSamplerBitfield ) )
    {
      // Reset to system default option
