...
TransformId TransformManager::CreateTransform()
{
  if( mTxComponent.empty() || mTxComponent.size() == mComponentCount )
  {
    ...
    mPositionDirty.resize(mComponentCount+1);
    mScaleDirty.resize(mComponentCount+1);
    mOrientationDirty.resize(mComponentCount+1);
  }
  ...
  mPositionDirty[mComponentCount] = 0u;
  mScaleDirty[mComponentCount] = 0u;
  mOrientationDirty[mComponentCount] = 0u;
...
void TransformManager::RemoveTransform(TransformId tx)
{
  ...
  if( index < mComponentCount-1 )
  {
    ...
    mPositionDirty[index] = mPositionDirty[mComponentCount-1];
    mScaleDirty[index] = mScaleDirty[mComponentCount-1];
    mOrientationDirty[index] = mOrientationDirty[mComponentCount-1];
...
void TransformManager::Update()
{
  ...
  for( unsigned int i(0); i<mComponentCount; ++i )
  {
    if( mParent[i] != INVALID_TRANSFORM_ID)
    {
      ...
      if( mPositionDirty[i] || mScaleDirty[i] || mOrientationDirty[i] )
      {
        Vector3 position = mTxComponent[i].mPosition +
            mTxComponent[i].mParentOrigin *  mTxComponent[parentIndex].mSize +
            mTxComponent[i].mAnchorPoint * mTxComponent[i].mSize * mTxComponent[i].mScale;
  ...
  memset( &mPositionDirty[0], 0u, sizeof(bool)*mComponentCount );
  memset( &mScaleDirty[0], 0u, sizeof(bool)*mComponentCount );
  memset( &mOrientationDirty[0], 0u, sizeof(bool)*mComponentCount );
...
void TransformManager::SwapComponents( unsigned int i, unsigned int j )
{
  ...
  std::swap( mPositionDirty[i], mPositionDirty[j] );
  std::swap( mScaleDirty[i], mScaleDirty[j] );
  std::swap( mOrientationDirty[i], mOrientationDirty[j] );
