...
void PropertyBuffer::SetData(  BufferIndex bufferIndex, PropertyBufferDataProvider::BufferType* data )
{
  ...
  mRenderBufferData[bufferIndex] = data;
}
...
{
  DALI_ASSERT_DEBUG( mRenderBufferData[bufferIndex] && "Should have some data.");

  return *mRenderBufferData[bufferIndex];
}
...
void PropertyBuffer::ResetDefaultProperties( BufferIndex updateBufferIndex )
{
  ...
  mRenderBufferData[updateBufferIndex] = mRenderBufferData[ 1 - updateBufferIndex];
}
