...
private:
  ...
  /**
   * Clear all the current layout and resources of the atlas
   */
  void ClearCache();
...
private:
  ...
  struct Layout
  {
    Layout( unsigned int xOffset, unsigned int yOffset, unsigned int sourceIndex, bool isBufferImage )
  ...
  private:
    Layout(const Layout& rhs); ///< not defined
    Layout& operator=(const Layout& rhs); ///< not defined
  };
