...
/**
 * Helper to set the depth function from a DALi DepthFunction enum
 * @param[in] context The GL context
 * @param[in] depthFunction The depth function
 */
inline void SetDepthFunction( Context& context, DepthFunction::Type depthFunction )
{
  context.DepthFunc( DaliDepthToGLDepthTable[ depthFunction ] );
}
...
inline void SetRenderFlags( const RenderList& renderList, Context& context, bool depthTestEnabled, bool isLayer3D )
{
  ...
  // Clear depth and, if required, stencil buffer.
  // Note: The buffers will only be cleared if written to since a previous clear.
  // The depth buffer will then be enabled / disabled per renderer after the clear IF we have color renderers.
  // If we do not have color renderers, this is only done for 3D layers.
  // DepthTest must be enabled for the layer.
  if( depthTestEnabled && ( renderList.HasColorRenderItems() || isLayer3D ) )
  {
    clearMask |= GL_DEPTH_BUFFER_BIT;
    // We need to enabled the depth buffer to clear it.
  ...
  const Matrix& projectionMatrix )
{
  ...
  if( renderList.HasColorRenderItems() && depthTestEnabled )
  {
