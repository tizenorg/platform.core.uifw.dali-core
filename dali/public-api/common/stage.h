...
public:
  ...
  typedef Signal< void (const TouchEvent&) > TouchedSignalType;          ///< @DEPRECATED_1_1.38 @brief Touched signal type @SINCE_1_0.0
  typedef Signal< void (const TouchEventHandle&) > TouchEventSignalType; ///< Touched signal type @SINCE_1_1.36
  ...
  /**
   * @DEPRECATED_1_1.36 Use TouchEventSignal() instead.
   ...
   * A callback of the following type may be connected:
   * @code
   *   void YourCallbackName( const TouchEventHandle& event );
   * @endcode
   *
   * @SINCE_1_1.36
   * @return The touch signal to connect to.
