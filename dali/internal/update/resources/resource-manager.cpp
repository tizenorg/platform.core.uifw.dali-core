...
void ResourceManager::PrintRenderOnceDebug( ResourceId id ) const
{
  for( unsigned int i=0; i<mDebugContainer.size(); ++i )
  {
    if( mDebugContainer[i].id == id )
    {
      DALI_LOG_ERROR( "RenderTask Waiting for resource %d type %s path %s\n", mDebugContainer[i].id, mDebugContainer[i].type.c_str(), mDebugContainer[i].path.c_str() );
