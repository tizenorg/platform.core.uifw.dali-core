...
public:
  ...
  {
    std::cout << "todor: SignalDelegate Constructor" << std::endl;
  }
  ...
  ~SignalDelegate()
  {
    std::cout << "todor: SignalDelegate Destructor" << std::endl;
  }
  ...
  {
    /**
     * Create and initialise the functor with the callback to be called.
     *
    ...
    ~CallbackBaseFunctor()
    {
      std::cout << "todor: CallbackBaseFunctor Destructor" << std::endl;
      delete mCallback;
    }
...
#if 0
  /**
   * TODO: Remove this, not needed as a FunctorDelegate can be converted to a CallbackBase.
   */
  struct FunctorDelegateDispatcher
  {
    FunctorDelegateDispatcher( FunctorDelegate* functorDelegate )
    : mFunctorDelegate( functorDelegate )
    {
      std::cout << "todor: FunctorDelegateDispatcher Constructor: " << (void *)functorDelegate << std::endl;
    }
    ...
    void operator()()
    {
      if( mFunctorDelegate )
      {
        std::cout << "todor: FunctorDelegateDispatcher Running callback" << std::endl;
        //CallbackBase::Execute( *mCallback );
        mFunctorDelegate->Execute();
      }
    }

    ~FunctorDelegateDispatcher()
    {
      std::cout << "todor: FunctorDelegateDispatcher Destructor" << std::endl;
      delete mFunctorDelegate;
    }

    private:
      FunctorDelegate* mFunctorDelegate;
  };
#endif
   ...
   *
   * TODO: more info & params
   */
  bool Connect( ConnectionTrackerInterface* connectionTracker, FunctorDelegate* functorDelegate )
  {
    std::cout << "todor: SignalDelegate::Connect( FunctorDelegate* ): actor:" << mConnectActor.GetName() << ", signal:" << mSignalName << std::endl;

    if( !mCallbackFunctor )
    {
      //mConnectActor.ConnectSignal( connectionTracker, mSignalName, reinterpret_cast< void (*)( void ) >( functor->GetFunctorPointer() ) );
      //boost::function< void ( void ) > handler = boost::bind( &SignalDelegate::SignalHandler, this );//todor replace Signal type
      //mFunctorDelegateDispatcher = new FunctorDelegateDispatcher( functorDelegate );
  ...
  {
    std::cout << "todor: SignalDelegate::Connect( object::memberfunction )" << std::endl;
  ...
  // TODO: delete
  template <class T>
  void Connect( ConnectionTrackerInterface* connectionTracker, const T& functor )
  {
    std::cout << "todor: SignalDelegate::Connect( functor )" << std::endl;
    //mCallbackFunctor = MakeCallback( FunctorDelegate::New( functor ) );

    mConnectActor.ConnectSignal( connectionTracker, mSignalName, functor );
  }
#endif
