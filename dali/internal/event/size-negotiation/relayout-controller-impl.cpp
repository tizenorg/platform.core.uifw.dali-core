...
void RelayoutController::OnInitializationCompleted( IntrusivePtr< Stage > stage )
{
...
void RelayoutController::RequestPreRelayout( Dali::Actor actor )
{
  ...
  bool found = false;
  for( unsigned int i = 0, count = mPreRequestActors.Size(); i < count; ++i )
  {
    if( mPreRequestActors[ i ] == actorPtr )
    {
      found = true;
      break;
    }
  ...
  if( !found )
  {
    mPreRequestActors.PushBack( actorPtr );
  }
}
...
void RelayoutController::RequestRelayoutTree( Dali::Actor& actor )
{
  ...
  // Request pre-relayout on all the actors
  RequestPreRelayout( actor );
...
void RelayoutController::OnObjectDestroyed( const Dali::RefObject* object )
{
  ...
  FindAndZero( mPreRequestActors, object );
...
void RelayoutController::Relayout()
{
  ...
  if( mRelayoutFlag )
  {
    ...
    // Pre-relayout
    for( RawActorList::Iterator it = mPreRequestActors.Begin(), itEnd = mPreRequestActors.End(); it != itEnd; ++it )
    {
      BaseObject* baseObject = *it;

      // Need to test if actor is valid (could have been deleted and had the pointer cleared)
      if( baseObject )
      {
        Actor* actorImpl = reinterpret_cast< Actor* >( baseObject );
        actorImpl->OnPreRelayout();
      }
    }
