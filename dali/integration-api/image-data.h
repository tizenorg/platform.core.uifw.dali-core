...
public:
   ...
   * @param[in] pixelFormat The format of the image data in the buffer.
   **/
  static ImageDataPtr New( const size_t numBytes, unsigned imageWidth, unsigned imageHeight, Pixel::Format pixelFormat );
   ...
   * @param[in] pixelFormat The format of the image data in the buffer.
   **/
  static ImageDataPtr New( uint8_t * const imageBuffer, const size_t numBytes, unsigned imageWidth, unsigned imageHeight, Pixel::Format pixelFormat );
  ...
  /**
   * You now own this buffer. Act wisely and call delete[] on it one day.
   **/
...
public:
  const size_t        mDataSize;   ///< Number of bytes in buffer pointed at by mData
...
inline ImageDataPtr ImageData::New(  const size_t numBytes, unsigned imageWidth, unsigned imageHeight, Pixel::Format pixelFormat )
{
  DALI_ASSERT_DEBUG( numBytes > 0 && "Zero allocations are pointless if also harmless."  );
...
inline ImageDataPtr ImageData::New( uint8_t * const imageBuffer, const size_t numBytes, unsigned imageWidth, unsigned imageHeight, Pixel::Format pixelFormat )
{
  DALI_ASSERT_DEBUG( numBytes > 0 && "Zero-length buffers are pointless if also harmless."  );
  DALI_ASSERT_DEBUG( imageWidth > 0 && imageHeight > 0 && "Zero dimensioned images are pointless if also harmless." );
...
inline ImageDataPtr NewBitmapImageData( unsigned imageWidth, unsigned imageHeight, Pixel::Format pixelFormat )
{
  ...
  return ImageData::New( numBytes, imageWidth, imageHeight, pixelFormat );
}
...
///----- * @param[in] pixelFormat The format of the image data in the buffer.
///----- */
///-----inline ImageDataPtr NewBitmapImageData( uint8_t * imageBuffer, unsigned imageWidth, unsigned imageHeight, Pixel::Format pixelFormat )
///-----{
///-----  DALI_ASSERT_DEBUG( pixelFormat <= Pixel::BGRA8888 && "Pixel format must be an addressable one." );
///-----  const size_t numBytes = imageWidth * imageHeight * Pixel::GetBytesPerPixel( pixelFormat );
///-----
///-----  return ImageData::New( imageBuffer, numBytes, imageWidth, imageHeight, pixelFormat );
///-----}
