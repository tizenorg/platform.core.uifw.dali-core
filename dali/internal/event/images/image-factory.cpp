...
ResourceTicketPtr ImageFactory::Reload( Request& request )
{
  ...
  else // ticket still alive
  {
    ...
    const ImageAttributes& requestAttribs = *request.attributes;
    const ImageDimensions closestSize = Internal::ThreadLocalStorage::Get().GetPlatformAbstraction().GetClosestImageSize( request.url,
                                                                                       ImageDimensions( requestAttribs.GetSize().width, requestAttribs.GetSize().width ),
                                                                                       requestAttribs.GetScalingMode(), requestAttribs.GetFilterMode(), requestAttribs.GetOrientationCorrection() );
...
ResourceTicketPtr ImageFactory::IssueLoadRequest( const std::string& filename, const ImageAttributes* attr )
{
  ...
  if( attr )
  {
    dimensions = FromFloatVec2( attr->GetSize() );
    scalingMode = attr->GetScalingMode();
  ...
  BitmapResourceType resourceType( dimensions, scalingMode, samplingMode, orientation );
