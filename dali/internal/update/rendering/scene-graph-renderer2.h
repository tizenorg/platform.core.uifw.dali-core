...
public:
  ...
  void OnStageDisconnect(Node* node);
  ...
  bool IsFullyOpaque( BufferIndex updateBufferIndex, Node* node );
...
public: //Uniform map providervoid R3nderer::GetReadyAndComplete(bool& ready, bool& complete) const
...
inline void OnStageDisconnectMessage( EventThreadServices& eventThreadServices, const R3nderer& renderer, const Node* node )
{
  typedef MessageValue1< R3nderer, Node* > LocalType;
  ...
  // Construct message in the message queue memory; note that delete should not be called on the return value
  new (slot) LocalType( &renderer, &R3nderer::OnStageDisconnect, const_cast<Node*>(node) );
}
