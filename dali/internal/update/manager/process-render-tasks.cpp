                            ...
                            int clippingDepth )
{
  ...
  {
    if( CLIP_DEBUG ) std::cout << "found clipping mode -------------------------------------------------------------------------------------------------------------" << std::endl;
    ++currentClippingId;
    ++clippingDepth;
  }
  ...
  for( unsigned int i = 0; i < count; ++i )
  {
    ...
    if( ready ) // i.e. should be rendered (all resources are available)
    {
...
#endif
      if( DrawMode::OVERLAY_2D == inheritedDrawMode )
      {
      ...
      {
        //todor
        if( CLIP_DEBUG ) std::cout << "todor #### Building renderlist: node: x  rendererNum:" << i << "  curClipId: " << currentClippingId << "  curClipDepth: " << clippingDepth << "  name: " << renderer->GetName() << std::endl;
...
{
  if( CLIP_DEBUG ) std::cout << "todor: ProcessRenderTasks: START" << std::endl;
  ...
  for ( RenderTaskList::RenderTaskContainer::Iterator iter = taskContainer.Begin(); endIter != iter; ++iter )
  {
    ...
    if( CLIP_DEBUG ) std::cout << "todor: ProcessRenderTasks: OFFSCREEN:" << std::endl;
  ...
  for ( RenderTaskList::RenderTaskContainer::Iterator iter = taskContainer.Begin(); endIter != iter; ++iter )
  {
    ...
    if( CLIP_DEBUG ) std::cout << "todor: ProcessRenderTasks: ONSCREEN:" << std::endl;
    if ( !renderTask.ReadyToRender( updateBufferIndex ) )
    {
      // Skip to next task
      continue;
