...
void GpuBuffer::UpdateDataBuffer(GLsizeiptr size,const GLvoid *data, Usage usage)
{
  ...
  mContext.BindCopyWriteBuffer( mBufferId );
  ...
  if (mBufferCreated )
  {
    ...
    if (size <= mCapacity )
    {
      mContext.BufferSubData( GL_COPY_WRITE_BUFFER, 0, size, data );
    }
    else
    {
      // create a new buffer of the larger size,
      // gl should automatically deallocate the old buffer
      mContext.BufferData( GL_COPY_WRITE_BUFFER, size, data, ModeAsGlEnum( usage ) );
      mCapacity = size;
    }
  }
  else
