...
void RenderTask::Initialize( RenderMessageDispatcher& renderMessageDispatcher, CompleteStatusManager& completeStatusManager )
{
...
void RenderTask::UpdateState()
{
  ...
  switch( mState )
  {
    ...
    case RENDERED_ONCE:
    {
      ...
      {
        if( this->mTargetIsNativeFramebuffer && mRenderSyncTracker )
        {
          mWaitingToRender = mRenderSyncTracker->IsSynced();
          mNotifyTrigger = true;
        }
        else if( mCompleteStatusManager && mCompleteStatusManager->HasFrameBufferBeenRenderedTo( mFrameBufferResourceId ) )
        {
