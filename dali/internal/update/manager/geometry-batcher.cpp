...
inline void mul_vec2mat4(Dali::Vector2& result, const Dali::Matrix& lhs, const Dali::Vector2& rhs)
{
...
inline void mul_vec3mat4(Dali::Vector3& result, const Dali::Matrix& lhs, const Dali::Vector3& rhs)
{
  const float *matrix = (const float*)&lhs;
...
inline void mul_vec4mat4(Dali::Vector4& result, const Dali::Matrix& lhs, const Dali::Vector4& rhs)
{
  const float *matrix = (const float*)&lhs;
...
typedef void(*fn_mul)( void*, const void*, const void*);
                                        ...
                                        unsigned int bufferIndex )
{
  ...
  for( i = 0; i < count; ++i )
  {
    ...
    if( !batchable || !currentBatchInfo || currentBatchInfo != info ) // nobatch, or batch swapped
    {
      ...
      if( currentBatchInfo )
      {
        ...
        batchMarkers.push_back( BatchStreamMarker( *mBatchMap[ crc ], startBatchIndex, i ) );
      }
  ...
  if( currentBatchInfo )
  {
    ...
    mBatchMap[ crc ]->mTouched = true;
    batchMarkers.push_back( BatchStreamMarker( *mBatchMap[ currentBatchInfo.mCRC ], startBatchIndex, i ) );
  }
  ...
  for( ; it != end; ++it )
  {
    ...
    printf("Batch crc: 0x%X, batchParent = %p, size = %d, mat = %p, shader = %p\n", info.mCRC, info.mBatchParent, (int)(marker.to - marker.from), info.mTextureSet, info.mShader );
   ...
   // }
    ...
    for( size_t k = marker.from; k < marker.to; ++k )
    {
      ...
      if( info.mTouched )
      {
        ...
        // save vertices
        memcpy(&outVertexPtr[ vertexIter ], inVertexPtr, inVertexBufferSize );
        TransformVertices( &outVertexPtr[ vertexIter ], inVertexPtr, &item->GetNode(), info.mVertexComponentSize, inVertexCount, bufferIndex );
...
void GeometryBatcher::GenerateMarkers( RenderItem **ppItems, size_t count, BufferIndex bufferIndex )
{
...
void GeometryBatcher::TransformVertices( void* dst, void* src, const Node* node, size_t componentSize, size_t numVerts, unsigned int bufferIndex )
{
  ...
  {
    Vector4& pos4 = *(Vector4*)source;
    Vector4 &finalPos4 = *(Vector4*)dest;
    mul_vec4mat4( finalPos4, finalMatrix, pos4 );
...
{
  if( !mDiscardList.empty() )
  {
    const size_t size = mDiscardList.size();
    printf("Discarding %d batches\n", (int)size );
    for( size_t i = 0; i < size; ++i )
    {
      BatchInfo* info = mBatchMap[ mDiscardList[i] ];
      updateManager->RemovePropertyBuffer( info->mVertexBuffer );
      mBatchMap.erase( mDiscardList[i] );
      delete info;
    }
    mDiscardList.clear();
  }
...
void GeometryBatcher::Update( UpdateManager* updateManager, BufferIndex bufferIndex, RenderInstructionContainer& instructions )
{
  ...
  for( BatchMap::iterator it = mBatchMap.begin(); it != mBatchMap.end(); ++it )
  {
    ...
    {
      //mDiscardList.push_back( it->first );
    }
  }

  printf("Keys to remove: %d\n", (int)mDiscardList.size());
}

void GeometryBatcher::DiscardBatch( Node* node )
{
  BatchMap::iterator it = mBatchMap.begin();
  BatchMap::iterator end = mBatchMap.end();
  for( ; it != end; ++it )
  {
    BatchInfo* info = it->second;
    if( info->mBatchParent == node )
    {
      mDiscardList.push_back( info->mCRC );
    }
  }
}
