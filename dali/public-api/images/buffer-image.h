 ...
 * SignalUploaded. If a pixel buffer is to be updated afterwards
 * then the pixel data must not be freed.
 *
...
public:
   ...
   * the external buffer to remain while an Update() could occur. Dali will create
   * a copy of the external buffer when the BufferImage is created and so Update()
   * should not be called afterwards if that external buffer is destroyed.
   *
