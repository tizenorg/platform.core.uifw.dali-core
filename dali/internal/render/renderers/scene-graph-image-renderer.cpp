...
bool ImageRenderer::IsOutsideClipSpace( const Frustum& frustum, const Matrix& modelMatrix, const Matrix& modelViewProjectionMatrix )
{
  mContext->IncrementRendererCount();
  if(IsOutsideClipSpaceImpl( frustum, modelMatrix, modelViewProjectionMatrix ) )
  {
...
bool ImageRenderer::IsOutsideClipSpaceImpl( const Frustum& frustum, const Matrix& modelMatrix, const Matrix& modelViewProjectionMatrix)
{
  ...
  Vector3 halfExtent = mDataProvider.GetHalfExtent();
  Vector3 center = mDataProvider.GetCenter();

  float dL = frustum[LEFT_PLANE].DistanceFromPoint(center);
  float rL = frustum[LEFT_PLANE].mAbsoluteNormal.Dot(halfExtent);
  if( dL+rL < 0 ) // fully outside
  {
    return true;
  }
  float dR = frustum[RIGHT_PLANE].DistanceFromPoint(center);
  float rR = frustum[RIGHT_PLANE].mAbsoluteNormal.Dot(halfExtent);
  if( dR+rR < 0 ) // fully outside
  {
    return true;
  }
  float dT = frustum[TOP_PLANE].DistanceFromPoint(center);
  float rT= frustum[TOP_PLANE].mAbsoluteNormal.Dot(halfExtent);
  if( dT+rT < 0 ) // fully outside
  {
    return true;
  }
  float dB = frustum[BOTTOM_PLANE].DistanceFromPoint(center);
  float rB = frustum[BOTTOM_PLANE].mAbsoluteNormal.Dot(halfExtent);
  if( dB+rB < 0 ) // fully outside
  {
    return true;
  }
