...
#if 1
inline void SetupStencilBuffer( const RenderItem& item, Context& context, int index, bool& hitClippingRenderer )
{
#if 0
  // DISABLE tmp stencil code
#if 1
  ...
  int stencilDepthC = item.GetClippingId();
  ...
  bool stencilBufferEnabled = false;
  Dali::Renderer::ClippingMode clippingMode = item.GetClippingMode();
  int clippingId = item.GetClippingId();
