  ...
  if( actor.GetChildCount() > 0 )
  {
    ...
    for( ActorIter iter = children.begin(), endIter = children.end(); iter != endIter; ++iter )
    {
           ...
           ( isStencil || hitCheck.DescendActorHierarchy( &GetImplementation( *iter ) ) ) ) // We are a stencil OR we can descend into child hierarchy
      {
        ...
        // If the actor was hit and its parent isn't renderable, then it should be preferred
        if ( !parentIsRenderable && updateChildHit )
        {
          childHit = currentHit;
        }

        // Otherwise, if the hit actor is more forward then the parent or the depth index is greater then that is preferred
        else if ( updateChildHit )
        {
          if ( currentHit.depth > hit.depth ||
