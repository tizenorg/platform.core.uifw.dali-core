...
void UpdateManager::ApplyConstraintsToRenderTasks( BufferIndex bufferIndex )
{
...
void UpdateManager::ApplyConstraintsToShaders( BufferIndex bufferIndex )
{
                                    ...
                                    unsigned int nextVSyncTimeMilliseconds )
{
  ...
  if( updateScene || mImpl->previousUpdateScene )
  {
    ...
    // 12) Update node hierarchy, apply constraints and perform sorting / culling.
    //This will populate each Layer with a list of renderers which are ready.
    UpdateNodes( bufferIndex );

    //Aply constraints to RenderTashsm, shaders and geometries
    ApplyConstraintsToRenderTasks( bufferIndex );
    ApplyConstraintsToShaders( bufferIndex );
