                         ...
                         unsigned int width, unsigned int height )
{
  if( mNativeImage )
  {
    DALI_LOG_ERROR( "Uploading data to a native texture is not permitted");
  }
  else
  {
    ...
    {
      DALI_LOG_ERROR( "PixelData doesn't contain any data");
    }
    else
    {
      ...
      if( ( pixelDataFormat == mFormat ) || ( (pixelDataFormat == Pixel::RGB888 ) && ( mFormat == Pixel::RGBA8888 ) ) )
      {
        ...
        {
          DALI_LOG_ERROR( "Texture upadate out of bounds");
        }
        else
        {
