...
public:
  ...
  /**
   * Called by AnimatorConnector when its ProxyObject has added the object to the scene.
   */
  void SceneObjectAdded()
  {
    mEnabled = true;
  }

  /**
   * Called by AnimatorConnector when its ProxyObject has been destroyed.
   * This is only needed because PropertyOwner will not notify of the destruction to its observers if the object
   * was not on stage.
   */
  virtual void SceneObjectDestroyed()
  {
  }
   ...
   * @param[in] bake Bake.
   * @return True if the update was applied, false if the animator is not enabled
   */
...
public:
  ...
  virtual void SceneObjectDestroyed()
  {
    if( mPropertyOwner )
    {
      PropertyOwnerDestroyed( *mPropertyOwner );
    }
  }
