...
void NewTexture::Upload( Context& context, Vector<unsigned char>& buffer, const Dali::TextureUploadParams& params  )
{
  ...
  {
    context.Bind2dTexture( mId );
    std::cout<<params.layer<<","<<params.x0<<","<<params.y0<<","<<params.width<<","<<params.height<<std::endl;
    context.TexSubImage2D( GL_TEXTURE_2D, params.layer, params.x0, params.y0, params.width, params.height, mInternalFormat, mPixelDataType, &buffer[0] );

    //FERRAN : mipmaps are regenerated when uploading layer 0...Maybe we should add a parameter to specify whether or not mipmaps should be automatically generated?
    if( params.layer == 0 && mMipmapCount > 0u )
    {
      context.GenerateMipmap( GL_TEXTURE_2D );
    }
  }
...
void NewTexture::ApplySampler( Context& context, Render::Sampler* sampler )
{
  ...
  if( mSampler.mBitfield != oldSampler.mBitfield )
  {
    ...
    {
      std::cout<<"Changing min filter"<<std::endl;
    ...
    if( mType == TextureType::TEXTURE_CUBE && mSampler.mUWrapMode != oldSampler.mUWrapMode )
    {
      GLint glWrapMode = WrapModeToGL( mSampler.mUWrapMode, GL_WRAP_DEFAULT );
