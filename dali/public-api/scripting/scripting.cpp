...
#include <dali/public-api/images/url-image.h>
...
const StringEnum< UrlImage::LoadPolicy > IMAGE_LOAD_POLICY_TABLE[] =
{
  { "IMMEDIATE", UrlImage::IMMEDIATE },
  { "ON_DEMAND", UrlImage::ON_DEMAND },
};
...
Image NewImage( const Property::Value& map )
{
  ...
  std::string filename;
  UrlImage::LoadPolicy loadPolicy    = Dali::Internal::IMAGE_LOAD_POLICY_DEFAULT;
  ...
  if( Property::MAP == map.GetType() )
  {
    ...
    if( map.HasKey(field) )
    {
      ...
      loadPolicy = GetEnumeration< UrlImage::LoadPolicy >( v, IMAGE_LOAD_POLICY_TABLE, IMAGE_LOAD_POLICY_TABLE_COUNT );
    }
    ...
    if( map.HasKey("type") )
    {
      ...
      {
        ret = UrlImage::New(filename, attributes, loadPolicy, releasePolicy);
      }
      else
      {
