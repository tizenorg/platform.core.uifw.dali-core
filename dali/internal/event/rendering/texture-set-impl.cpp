...
{
  TextureSetPtr imageArray( new TextureSet() );
...
{
  ResourceId resourceId = Integration::InvalidResourceId;
  if( image )
  {
  ...
  if( mTextures.size() < index + 1  )
  {
    mTextures.resize(index + 1);
  }

  mTextures[index].image = image;
  SceneGraph::SetImageMessage( GetEventThreadServices(), *mSceneObject, index, resourceId );
}
...
const SceneGraph::TextureSet* TextureSet::GetTexturesSceneObject() const
{
  return mSceneObject;
}
...
void TextureSet::Connect()
{
  ...
  for( size_t i(0); i<mTextures.size(); ++i )
  {
    if( mTextures[i].image )
    {
      ...
      if( mTextures[i].image->GetResourceId() != 0 )
      {
