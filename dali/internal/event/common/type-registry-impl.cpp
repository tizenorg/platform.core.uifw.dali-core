...
bool TypeRegistry::RegisterAnimatableProperty( TypeRegistration& registered, const std::string& name, const Property::Type& type )
{
  RegistryMap::iterator iter = mRegistryLut.find( registered.RegisteredName() );
  ...
  if( iter != mRegistryLut.end() )
  {
    ...
    GetImplementation(iter->second).AddAnimatableProperty( name, type );
