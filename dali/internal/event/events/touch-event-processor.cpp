 ...
 */
Dali::Actor EmitTouchSignals( Actor* actor, RenderTask& renderTask, const TouchEvent& originalEvent, const IntrusivePtr< TouchData >& originalTouchData, PointState::Type state )
{
  Dali::Actor consumingActor;

  if( actor )
  {
    IntrusivePtr< TouchData > touchData = TouchData::Clone( *originalTouchData.Get() );
...
void TouchEventProcessor::ProcessTouchEvent( const Integration::TouchEvent& event )
{
  ...
  if ( event.points[0].GetState() == PointState::INTERRUPTED )
  {
    ...
    //currentPoint.SetHitActor( Actor() );
