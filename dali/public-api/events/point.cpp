...
struct Point::Impl
{
  ...
  int       deviceId;     ///< Each point has a unique device ID which specifies the device used for that point.
};

Point::Point( int deviceId, State state, Vector2 screen )
: mImpl( new Impl )
{
...
Point::Point( int deviceId, State state, Vector2 screen, Vector2 local )
: mImpl( new Impl )
{
  mImpl->deviceId = deviceId;
  mImpl->state = state;
...
Vector2 Point::GetLocal() const
{
  return mImpl->local;
}

Vector2 Point::GetScreen() const
{
  return mImpl->screen;
}
...
void Point::SetLocal( Vector2 local )
{
  mImpl->local = local;
}

void Point::SetScreen( Vector2 screen )
{
  mImpl->screen = screen;
}
