...
int UtcDaliRendererSetGetFaceCullingMode(void)
{
  ...
  DALI_TEST_CHECK( renderer.GetFaceCullingMode() == Renderer::NONE );
  ...
  {
    cullFaceStack.Reset();
    renderer.SetFaceCullingMode( Renderer::CULL_BACK_AND_FRONT );
    application.SendNotification();
    application.Render();
    ...
    DALI_TEST_CHECK( renderer.GetFaceCullingMode() == Renderer::CULL_BACK_AND_FRONT);
  }

  {
    cullFaceStack.Reset();
    renderer.SetFaceCullingMode( Renderer::CULL_BACK );
    application.SendNotification();
    application.Render();
    ...
    DALI_TEST_CHECK( cullFaceStack.FindMethodAndParams( "CullFace", cullModeString.str() ) );
    DALI_TEST_CHECK( renderer.GetFaceCullingMode() == Renderer::CULL_BACK );
  }

  {
    cullFaceStack.Reset();
    renderer.SetFaceCullingMode( Renderer::CULL_FRONT );
    application.SendNotification();
    application.Render();
    ...
    DALI_TEST_CHECK( cullFaceStack.FindMethodAndParams( "CullFace", cullModeString.str() ) );
    DALI_TEST_CHECK( renderer.GetFaceCullingMode() == Renderer::CULL_FRONT );
  }

  {
    cullFaceStack.Reset();
    renderer.SetFaceCullingMode( Renderer::NONE );
    application.SendNotification();
    application.Render();
...
int UtcDaliRendererSetBlendMode02(void)
{
  ...
  renderer.SetBlendMode(BlendingMode::OFF);
...
int UtcDaliRendererSetBlendMode03(void)
{
  ...
  renderer.SetBlendMode(BlendingMode::AUTO);
...
int UtcDaliRendererGetBlendMode(void)
{
  ...
  // default value
  DALI_TEST_EQUALS( renderer.GetBlendMode(), BlendingMode::AUTO, TEST_LOCATION );

  // ON
  renderer.SetBlendMode(BlendingMode::ON);
  DALI_TEST_EQUALS( renderer.GetBlendMode(), BlendingMode::ON, TEST_LOCATION );

  // OFF
  renderer.SetBlendMode(BlendingMode::OFF);
  DALI_TEST_EQUALS( renderer.GetBlendMode(), BlendingMode::OFF, TEST_LOCATION );
...
int UtcDaliRendererGetBlendColor(void)
{
  ...
  DALI_TEST_EQUALS( renderer.GetBlendColor(), Color::TRANSPARENT, TEST_LOCATION );

  renderer.SetBlendColor( Color::MAGENTA );
  application.SendNotification();
  application.Render();
  DALI_TEST_EQUALS( renderer.GetBlendColor(), Color::MAGENTA, TEST_LOCATION );

  Vector4 color( 0.1f, 0.2f, 0.3f, 0.4f );
  renderer.SetBlendColor( color );
  application.SendNotification();
  application.Render();
  DALI_TEST_EQUALS( renderer.GetBlendColor(), color, TEST_LOCATION );
...
int UtcDaliRendererPreMultipledAlpha(void)
{
  ...
  renderer.GetBlendFunc( srcFactorRgb, destFactorRgb, srcFactorAlpha, destFactorAlpha );
  DALI_TEST_EQUALS( PRE_MULTIPLIED_ALPHA_BLENDING_SRC_FACTOR_RGB,    srcFactorRgb,    TEST_LOCATION );
  DALI_TEST_EQUALS( PRE_MULTIPLIED_ALPHA_BLENDING_DEST_FACTOR_RGB,   destFactorRgb,   TEST_LOCATION );
  DALI_TEST_EQUALS( PRE_MULTIPLIED_ALPHA_BLENDING_SRC_FACTOR_ALPHA,  srcFactorAlpha,  TEST_LOCATION );
  DALI_TEST_EQUALS( PRE_MULTIPLIED_ALPHA_BLENDING_DEST_FACTOR_ALPHA, destFactorAlpha, TEST_LOCATION );
