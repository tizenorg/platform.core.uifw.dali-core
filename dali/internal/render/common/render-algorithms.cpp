...
Dali::Matrix gModelViewProjectionMatrix( false ); ///< a shared matrix to calculate the MVP matrix, dont want to store it in object to reduce storage overhead
  ...
  if( renderList.HasColorRenderItems() )
  {
    ...
    for ( size_t index = 0; index < count; ++index )
    {
      ...
      else // Render batched geometry
      {
        ...
        {
          // FIXME - pass in View * Projection instead
          Matrix::Multiply( gModelViewProjectionMatrix, item.GetModelViewMatrix(), projectionMatrix );
          batchInfo->program->SetUniformMatrix4fv( loc, 1, gModelViewProjectionMatrix.AsFloat() );
        }
