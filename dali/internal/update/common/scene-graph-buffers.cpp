...
BufferIndex SceneGraphBuffers::INITIAL_RENDER_BUFFER_INDEX = 1u;
BufferIndex SceneGraphBuffers::INITIAL_EVENT_BUFFER_INDEX  = 2u;

SceneGraphBuffers::SceneGraphBuffers()
: mEventBufferIndex(INITIAL_EVENT_BUFFER_INDEX),
  mRenderBufferIndex(INITIAL_RENDER_BUFFER_INDEX),
  mBalance(0)
{
...
{
  __sync_fetch_and_xor( &mEventBufferIndex, 3u );
...
void SceneGraphBuffers::SanityCheck()
{
  ...
  if ( ( updateBuffer && renderBuffer ) && ( updateBuffer == renderBuffer ) )
  {
    ...
    DALI_ASSERT_ALWAYS( false && "MessageBuffer conflict between Update and Render threads" );
