...
public:
  ...
  static TxManager& GetTxManager()
  {
    static TxManager txManager;
    return txManager;
  }
  /**
  ...
  void SetTransform( TxId tx, const Vector3& position, const Vector3& scale,  const Vector3& size, const Quaternion& orientation, const Vector3& parentOrigin, const Vector3& anchorPoint )
  {
    unsigned int index = mId[tx];
    ...
    mTxComponent[index].mParentOrigin = parentOrigin;
    mTxComponent[index].mAnchorPoint = anchorPoint;
  }
  ...
  void Update()
  {
    ...
    for( unsigned int i(0); i<mComponentCount; ++i )
    {
      if( mParent[i] != INVALID_ID )
      {
        ...
        Vector3 position = mTxComponent[i].mPosition + (mTxComponent[i].mParentOrigin - Vector3(0.5f,0.5f,0.5f)) * mTxComponent[ parentIndex ].mSize
                          + ( Vector3(0.5f,0.5f,0.5f) - mTxComponent[i].mAnchorPoint) * mTxComponent[i].mSize;
