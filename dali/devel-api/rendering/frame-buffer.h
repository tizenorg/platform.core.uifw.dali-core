...
public:
   ...
   * @param[in] face For cube maps indicates which face of the cube to attach. Unused for 2D textures
   * @note The specified texture mipmap has to have the same size than the FrameBuffer
   * otherwise it won't be attached
   */
  void AttachColorTexture( Texture& texture, unsigned int mipmapLevel = 0u, CubeMap::Face face = CubeMap::POSITIVE_X );
