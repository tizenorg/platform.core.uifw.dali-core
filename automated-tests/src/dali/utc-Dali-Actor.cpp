...
int UtcDaliActorSizeMode(void)
{
  ...
  application.SendNotification();
  application.Render();
  DALI_TEST_EQUALS( newChild2.GetCurrentSize(), Vector3( 5.0f, 5.0f, 5.0f ), Math::MACHINE_EPSILON_0, TEST_LOCATION );

  // Change the parent's size and check the child's size changes also.
  newParent2.SetSize( 100.0f, 100.0f, 100.0f );
  application.SendNotification();
  application.Render();
  DALI_TEST_EQUALS( newChild2.GetCurrentSize(), Vector3( 50.0f, 50.0f, 50.0f ), Math::MACHINE_EPSILON_0, TEST_LOCATION );
  ...
  application.SendNotification();
  application.Render();
  DALI_TEST_EQUALS( newChild2.GetCurrentSize(), Vector3( 200.0f, 200.0f, 200.0f ), Math::MACHINE_EPSILON_0, TEST_LOCATION );
  // Confirm the child's size is still correct on the next frame.
  application.Render();
  DALI_TEST_EQUALS( newChild2.GetCurrentSize(), Vector3( 200.0f, 200.0f, 200.0f ), Math::MACHINE_EPSILON_0, TEST_LOCATION );
