...
struct TransformVertexBufferData
{
  ...
  const float*                  transform;            ///< transform relative to batch parent
  ...
  if( !mImpl->batchParents.empty() )
  {
    ...
    for( size_t batchParentIndex = 0; iter != end; ++iter, ++batchParentIndex )
    {
      ...
      for( size_t i = 0; i < size; ++i )
      {
        ...
        Dali::Internal::MultiplyMatrices( transform, worldMatrix.AsFloat(), invWorldMatrix.AsFloat() );
        ...
        transformParameters.transform = transform;
