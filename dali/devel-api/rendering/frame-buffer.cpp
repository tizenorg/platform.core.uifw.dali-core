...
void FrameBuffer::AttachColorTexture( Texture& texture, unsigned int mipmapLevel, CubeMap::Face face )
{
  if( texture )
  {
    Internal::NewTexturePtr texturePtr( &GetImplementation( texture ) );
    GetImplementation(*this).AttachColorTexture( texturePtr, mipmapLevel, face );
  }
}
