...
void HoverEventProcessor::ProcessHoverEvent( const Integration::HoverEvent& event )
{
  ...
  if( (primaryPointState == TouchPoint::Motion) || (primaryPointState == TouchPoint::Finished) || (primaryPointState == TouchPoint::Stationary) )
  {
    if ( mLastRenderTask )
    {
      ...
      {
        if( lastPrimaryHitActor->IsHittable() && MultiPointEventUtil::IsActuallySensitive( lastPrimaryHitActor ) )
        {
      ...
      {
        if( lastConsumedActor->IsHittable() && MultiPointEventUtil::IsActuallySensitive( lastConsumedActor ) )
        {
