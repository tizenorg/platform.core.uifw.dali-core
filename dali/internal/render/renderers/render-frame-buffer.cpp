...
void FrameBuffer::AttachColorTexture( Context& context, Render::NewTexture* texture, unsigned int mipmapLevel, unsigned int face )
{
  context.BindFramebuffer( GL_FRAMEBUFFER, mId );
  ...
  {
    context.FramebufferTexture2D( GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, texture->GetId(), mipmapLevel );
  }
