...
template< typename T >
void TestImage( Property::Map& map, const StringEnum* values, unsigned int num, T (Image::*method)() const )
{
  ...
  for ( unsigned int i = 0; i < num; ++i )
  {
    ...
    Image image = NewImage( map );
    DALI_TEST_EQUALS( values[i].value, (image.*method)(), TEST_LOCATION );
  }
}

template< typename T >
void TestImageAttributes( Property::Map& map, const StringEnum* values, unsigned int num, T (ImageAttributes::*method)() const )
{
  Property::Map::iterator iter = map.end() - 1;
  for ( unsigned int i = 0; i < num; ++i )
  {
    ...
    DALI_TEST_EQUALS( values[i].value, (attributes.*method)(), TEST_LOCATION );
  }
}
...
void UtcDaliScriptingNewImage()
{
  ...
  map.push_back( Property::StringValuePair( "load-policy", "" ) );
  {
    ...
    TestImage< Image::LoadPolicy >( map, values, ( sizeof( values ) / sizeof ( values[0] ) ), &Image::GetLoadPolicy );
  }
  ...
  map.push_back( Property::StringValuePair( "release-policy", "" ) );
  {
    ...
    TestImage< Image::ReleasePolicy >( map, values, ( sizeof( values ) / sizeof ( values[0] ) ), &Image::GetReleasePolicy );
  }
  ...
  map.push_back( Property::StringValuePair( "pixel-format", "" ) );
  {
    ...
    TestImageAttributes< Pixel::Format >( map, values, ( sizeof( values ) / sizeof ( values[0] ) ), &ImageAttributes::GetPixelFormat );
  }
  ...
  map.push_back( Property::StringValuePair( "scaling-mode", "" ) );
  {
    ...
    TestImageAttributes< ImageAttributes::ScalingMode >( map, values, ( sizeof( values ) / sizeof ( values[0] ) ), &ImageAttributes::GetScalingMode );
  }
