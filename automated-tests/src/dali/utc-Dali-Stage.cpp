...
int UtcDaliStageInitializeDynamicsP(void)
{
  ...
  try
  {
    DALI_TEST_CHECK( stage.InitializeDynamics( config ) );
  }
  catch( Dali::DaliException& e )
  {
    DALI_TEST_PRINT_ASSERT( e );
...
int UtcDaliStageInitializeDynamicsN(void)
{
#if !defined(DYNAMICS_SUPPORT)
  tet_infoline( "No dynamics support compiled\n" );
  return 0;
  ...
  // Check that getting the object registry with a bad stage instance causes an assert.
  bool asserted = false;
  try
  {
    stage.InitializeDynamics( config );
  }
  catch( Dali::DaliException& e )
  {
    DALI_TEST_PRINT_ASSERT( e );
...
int UtcDaliStageGetDynamicsWorldP(void)
{
#if !defined(DYNAMICS_SUPPORT)
  tet_infoline( "No dynamics support compiled\n" );
  return 0;
  ...
  {
    stage.InitializeDynamics( config );
    DynamicsWorld world = stage.GetDynamicsWorld();
...
int UtcDaliStageGetDynamicsWorldN(void)
{
#if !defined(DYNAMICS_SUPPORT)
  tet_infoline( "No dynamics support compiled\n" );
  return 0;
...
int UtcDaliStageTerminateDynamicsP(void)
{
#if !defined(DYNAMICS_SUPPORT)
  tet_infoline( "No dynamics support compiled\n" );
  return 0;
...
int UtcDaliStageTerminateDynamicsN(void)
{
#if !defined(DYNAMICS_SUPPORT)
  tet_infoline( "No dynamics support compiled\n" );
  return 0;
