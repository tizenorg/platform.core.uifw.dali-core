...
public:
  ...
  virtual ~AnimatorConnector()
  {
    ...
    //after AnimatorConnectors are destroyed (either when parent animation is destroyed or in its the Clear() method).
  }
...
private:
  ...
  virtual void SceneObjectAdded( ProxyObject& proxy )
  {
    ...
    //The object has been added to the scene. Notify the Scenegraph::Animator
    mAnimator->SceneObjectAdded();
  }
  ...
  virtual void ProxyDestroyed( ProxyObject& proxy )
  {
    ...
    //Notify the SceneGraph::Animator that the target object has been destroyed
    if(mAnimator)
    {
      mAnimator->SceneObjectDestroyed();
    }
  }
