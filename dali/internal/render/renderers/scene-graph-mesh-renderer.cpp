                                         ...
                                         ShaderSubTypes& outSubType )
{
  ...
  if( mShader->AreSubtypesRequired( outType ) )
  {
    ...
    GLsizei numBoneMatrices = GLsizei(meshData.GetBoneCount());
                             ...
                             const Matrix& viewMatrix )
{
  ...
  mesh.UploadVertexData( *mContext );
  mesh.BindBuffers( *mContext );

  GLsizei numBoneMatrices = GLsizei(mesh.GetMeshData().GetBoneCount());
  ...
  if( numBoneMatrices > 0 )
  {
    ...
    if( mesh.GetMeshData().HasNormals() )
    {
  ...
  else if( mesh.GetMeshData().HasColor() )  // Normals & color are mutually exclusive
  {
  ...
  Dali::MeshData::VertexGeometryType vertexGeometry = mesh.GetMeshData().GetVertexGeometryType();
