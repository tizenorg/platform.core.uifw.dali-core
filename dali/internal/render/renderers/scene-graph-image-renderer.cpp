...
void ImageRenderer::SetNinePatchMeshData( Texture* texture, const Vector2& size, const Vector4& border, bool borderInPixels, const PixelArea* pixelArea, bool noCenter )
{
  ...
  /*
   * If the center part is not going to be rendered, the draw order of the span
   * is 1->4->7->8->9->6->3->2.
   *
   *  |---|---------------|---|
   *  | 7 |    --> 8      | 9 |
  ...
  Vertex2D* verts = noCenter ? vertsWithNoCenter : vertsWithCenter;
  const size_t vertsSize = noCenter ? sizeof( vertsWithNoCenter ) : sizeof( vertsWithCenter );
