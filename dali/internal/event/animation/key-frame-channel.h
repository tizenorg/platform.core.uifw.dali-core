...
V KeyFrameChannel<V>::GetValue (float progress, Dali::Animation::Interpolation interpolation) const
{
  ...
  else if(FindInterval(start, end, progress))
  {
    ...
    {
      interpolatedV = Interpolate(start->GetValue(), end->GetValue(), frameProgress);
    }
    else
    {
      ...
      {
        //Project prev value throug end point
        next = end->GetValue() + (end->GetValue()-(end-1)->GetValue());
      }

      interpolatedV = CubicInterpolate(prev, start->GetValue(), end->GetValue(), next, frameProgress);
    }
  }
