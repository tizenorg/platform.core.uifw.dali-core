...
StyledCharacters::StyledCharacters( std::size_t firstCharacter, std::size_t numberOfCharacters, const TextStyle& style )
: mImpl( new Impl() )
{
  ...
  // Register the style.
  mImpl->mStyleItem = Internal::TextStyleManager::Get().RegisterTextStyle( style );
  // ++gStyledCharacters;
  // std::cout << "-----------------------------------------StyledCharacters : " << gStyledCharacters << std::endl;
}
...
{
  DALI_ASSERT_ALWAYS( mImpl && "StyledCharacters::GetTextStyle. Can't retrieve the style if any has been set." );
