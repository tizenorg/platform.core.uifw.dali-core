...
int UtcDaliResourceImageNew02(void)
{
  ...
  tet_infoline("UtcDaliREsourceImageNew02 - ResourceImage New( const std::string& url, ImageDimensions size, ScalingMode scalingMode, SamplingMode samplingMode, bool orientationCorrection = true )");
  ...
  // initialise handle
  image = ResourceImage::New(gTestImageFilename, ImageDimensions( 128, 256 ), ScalingMode::FitHeight, SamplingMode() );
