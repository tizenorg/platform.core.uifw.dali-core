...
inline void mul_vecsize( Dali::Vector2& result, const Dali::Vector2& lhs, const Dali::Vector3& size )
{
  result.x = lhs.x * size.x;
  result.y = lhs.y * size.y;
}

inline void mul_vecsize( Dali::Vector3& result, const Dali::Vector3& lhs, const Dali::Vector3& size )
{
  result.x = lhs.x * size.x;
  result.y = lhs.y * size.y;
...
inline void mul_vecsize( Dali::Vector4& result, const Dali::Vector4& lhs, const Dali::Vector3& size )
{
  result.x = lhs.x * size.x;
  result.y = lhs.y * size.y;
  result.z = lhs.z * size.z;
...
struct TransformVertexBufferInfo
{
  ...
  const Dali::Vector3   premulSize;           //< premultiply vertex size
};
...
void TransformVertexBuffer( const TransformVertexBufferInfo& info )
{
  ...
  if ( interleaved )
  {
    ...
    {
      mul_vecsize( *destination, *source, info.premulSize );
      mul_vecmat4( *destination, transform, *destination );
