...
struct TransformVertexBufferInfo
{
  ...
  size_t                componentSize;        //< size of component, may be 0 if not interleaved packing used
  size_t                vertexCount;          //< number of vertices to process
};
...
struct Impl
{
  ...
  typedef std::vector< BatchStreamMarker >    MarkerArray; //< using std::vector because of complex type
                                        ...
                                        unsigned int bufferIndex )
{
  ...
  for( ; i < count; ++i )
  {
    ...
    if( !batchable || !currentBatchInfo || currentBatchInfo != info ) // nobatch, or batch swapped
    {
      ...
      if( currentBatchInfo )
      {
        ...
        mImpl->mMarkers.push_back( BatchStreamMarker( *mImpl->mBatchMap[ crc ], startBatchIndex, i ) );
      }
  ...
  if( currentBatchInfo )
  {
    ...
    mImpl->mBatchMap[ crc ]->mTouched = true;
    mImpl->mMarkers.push_back( BatchStreamMarker( *mImpl->mBatchMap[ currentBatchInfo.mCRC ], startBatchIndex, i ) );
  }
  ...
  for( ; it != end; ++it )
  {
    ...
    for( size_t k = marker.from; k < marker.to; ++k )
    {
      ...
      const Matrix& modelMatrix = info.mBatchParent->GetWorldMatrix( bufferIndex );
