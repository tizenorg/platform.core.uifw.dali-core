...
const unsigned int UNINITIALISED_BITFIELD_PATTERN = ~0;
...
{
  // Start shifting from 1 as 0 is the uninitialised state
  switch ( mode )
  {
...
bool RetrieveFilterMode( unsigned int options, int mask, int bitShift, FilterMode::Type& filterModeOut )
{
  ...
  if( index != ( UNINITIALISED_BITFIELD_PATTERN & mask ) )
  {
...
{
  return UNINITIALISED_BITFIELD_PATTERN;
}
