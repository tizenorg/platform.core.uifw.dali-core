...
void Animation::SetPlayRange( const Vector2& range )
{
  ...
  else
  {
    ...
    // ensure that the animation stops.
    // If not yet at the start of the range, clamping will jump. @todo - Query if we
    // should instead ignore lower bound of range, and let animation smoothly transition
    // into range.
