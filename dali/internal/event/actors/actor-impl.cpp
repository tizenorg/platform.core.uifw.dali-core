...
void Actor::NotifySizeAnimation( Animation& animation, const Vector3& targetSize, Dimension::Type dimension )
{
  ...
  if( dimension == Dimension::ALL_DIMENSIONS )
  {
  ...
  for( unsigned int i = 0, count = GetChildCount(); i < count; ++i )
  {
    ...
    if( ( dimension & Dimension::WIDTH ) && childImpl.RelayoutDependentOnParent( Dimension::WIDTH ) )
    {
    ...
    if( ( dimension & Dimension::HEIGHT ) && childImpl.RelayoutDependentOnParent( Dimension::HEIGHT ) )
    {
