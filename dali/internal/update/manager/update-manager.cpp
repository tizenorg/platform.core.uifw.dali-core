...
void UpdateManager::AddNode( Node* node )
{
  ...
  if(DALI_UNLIKELY(mImpl->nodes.Empty()))
  {
    mImpl->nodes.PushBack(node);
  }
  else
  {
    Vector<Node*>::Iterator begin = mImpl->nodes.Begin();

    // case when node should be added as first
    bool added = false;
    ...
    // if node hasn't been added, insert it before first node
    if(DALI_UNLIKELY(!added))
    {
      mImpl->nodes.Insert(begin, node);
    }
  }
}
...
void UpdateManager::DestroyNode( Node* node )
{
  ...
  Vector<Node*>::Iterator iter = mImpl->nodes.Begin();
