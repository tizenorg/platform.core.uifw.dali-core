...
{
  if( IsRelayoutEnabled() && !Internal::RelayoutController::Get()->IsPerformingRelayout() )
  {
    if( width > 0.0f )
    {
      GetImplementation(*this).SetResizePolicy( FIXED, WIDTH );
    }

    if( height > 0.0f )
    {
      GetImplementation(*this).SetResizePolicy( FIXED, HEIGHT );
    }
