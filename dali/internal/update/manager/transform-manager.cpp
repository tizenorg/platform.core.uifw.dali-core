...
TransformId TransformManager::CreateTransform()
{
  if( mTxComponent.empty() || mTxComponent.size() == mComponentCount )
  {
    ...
    mPositionProperty.resize(mComponentCount+1);
    mScaleProperty.resize(mComponentCount+1);
    mOrientationProperty.resize(mComponentCount+1);
  }
  ...
  mScaleProperty[mComponentCount].mValue = Vector3(1.0f,1.0f,1.0f);
  mScaleProperty[mComponentCount].mBaseValue = Vector3(1.0f,1.0f,1.0f);
...
void TransformManager::RemoveTransform(TransformId tx)
{
  ...
  if( index < mComponentCount-1 )
  {
    ...
    mPositionProperty[index] = mPositionProperty[mComponentCount-1];
    mScaleProperty[index] = mScaleProperty[mComponentCount-1];
    mOrientationProperty[index] = mOrientationProperty[mComponentCount-1];
...
void TransformManager::Update()
{
  ...
  for( unsigned int i(0); i<mComponentCount; ++i )
  {
    if( mParent[i] != INVALID_TRANSFORM_ID)
    {
      ...
      if( !mPositionProperty[i].mIsClean )
      {
        mTxComponent[i].mPosition = mPositionProperty[i].mValue;
        mPositionProperty[i].ResetToBaseValue();
      }
      if( !mScaleProperty[i].mIsClean )
      {
        mTxComponent[i].mScale = mScaleProperty[i].mValue;
        mScaleProperty[i].ResetToBaseValue();
      }
      if( !mOrientationProperty[i].mIsClean )
      {
        mTxComponent[i].mOrientation = mOrientationProperty[i].mValue;
        mOrientationProperty[i].ResetToBaseValue();
      }
...
void TransformManager::SwapComponents( unsigned int i, unsigned int j )
{
  ...
  std::swap( mPositionProperty[i], mPositionProperty[j] );
  std::swap( mScaleProperty[i], mScaleProperty[j] );
  std::swap( mOrientationProperty[i], mOrientationProperty[j] );
