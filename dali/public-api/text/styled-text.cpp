...
struct StyledText::Impl
{
  ...
  void SetStyle( const TextStyle& style, std::size_t begin, std::size_t end, TextStyle::Mask mask )
  {
    ...
    mStyles.PushBack( new StyledCharacters( begin, 1u + end - begin, toBeMerged ) );
    // std::cout << "<--StyledText::Impl::SetStyle" << std::endl;
  }
  ...
  void Remove( std::size_t position, std::size_t numberOfCharacters )
  {
	...
	index -= numberOfCharacters;
      ...
      else if( ( position <= index ) && ( removeRange >= styleRange ) )
      {
  ...
  void Split( const Vector<std::size_t>& positions, Vector<StyledText*>& styledTexts ) const
  {
    ...
    for( Vector<std::size_t>::ConstIterator it = positions.Begin(), endIt = positions.End(); it != endIt; ++it )
    {
      ...
      styledTexts.PushBack( styledText );
  ...
  {
    Text text;
    mText.GetSubText( position, numberOfCharacters, text );

    StyledText* styledText = new StyledText( text );

    CopySubStyles( position, position + numberOfCharacters - 1u, styledText );
...
void StyledText::Split( const Vector<std::size_t>& positions, Vector<StyledText*>& styledTexts ) const
{
  if( NULL == mImpl )
  {
    // nothing to do.
