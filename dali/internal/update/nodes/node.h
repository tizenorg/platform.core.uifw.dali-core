...
public:
  ...
  void SetWorldMatrix( BufferIndex updateBufferIndex, const Vector3& scale, const Quaternion& rotation, const Vector3& translation )
  {
    ...
    // AB: temporarily here, if node may be batch parent, it will need inverted matrix
    if( mBatching )
    {
      mWorldInvertedMatrix.Get( updateBufferIndex ) = mWorldMatrix.Get( updateBufferIndex );
      mWorldInvertedMatrix.Get( updateBufferIndex ).Invert();
    }
  }
  ...
  const Matrix& GetWorldInvertedMatrix( BufferIndex bufferIndex ) const
  {
