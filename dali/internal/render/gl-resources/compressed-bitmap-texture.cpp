...
CompressedBitmapTexture::CompressedBitmapTexture(Internal::BitmapCompressed* const bitmap, Context& context, SceneGraph::TextureCache::DiscardPolicy discardPolicy)
: Texture(context,
...
void CompressedBitmapTexture::Update( Integration::Bitmap* bitmap )
{
  ...
  else
  {
    ...
    if ( mId ) // If the texture is already bound
    {
      ...
      if( mDiscardPolicy == SceneGraph::TextureCache::DISCARD )
      {
