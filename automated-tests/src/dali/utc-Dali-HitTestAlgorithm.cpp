...
int UtcDaliHitTestAlgorithmOrtho02(void)
{
  ...
  {
    HitTestAlgorithm::Results results;
    HitTest(stage, Vector2::ZERO + Vector2( 0.001f, 0.001f ), results, &DefaultIsActorTouchableFunction);
    DALI_TEST_CHECK( results.actor == blue );
    DALI_TEST_EQUALS( results.actorCoordinates, Vector2::ZERO, 0.002f, TEST_LOCATION );
  }
