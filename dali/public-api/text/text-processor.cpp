...
bool AreAllWhiteSpaces( const Integration::TextArray& text, std::size_t from, std::size_t numberOfCharacters )
{
  for( Integration::TextArray::ConstIterator it = text.Begin() + from, endIt = text.Begin() + ( from + numberOfCharacters ); it != endIt; ++it )
  {
                                    ...
                                    std::vector<TextProcessor::FontInfo>& fontInfoForCharacters )
{
  ...
  const Integration::TextArray& textArray = styledText.GetText().GetImplementation().GetTextArray();
  ...
  std::size_t index = 0u;
  for( Integration::TextArray::ConstIterator it = textArray.Begin(), endIt = textArray.End(); it != endIt; ++it, ++index )
  {
    TextStyle style;
