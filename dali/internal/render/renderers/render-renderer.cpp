...
{
  return new NewRenderer(nodeDataProvider, dataProviders);
}
...
void NewRenderer::SetDataProviders( DataProviders* dataProviders )
{
  mDataProviders = dataProviders;
...
{
  BindTextures( bufferIndex, program, mDataProviders->GetSamplers() );

  SetUniforms( bufferIndex, program );

  mRenderGeometry.UploadAndDraw( mContext, program, bufferIndex, mDataProviders.Get() );
}
...
void NewRenderer::SetUniforms( BufferIndex bufferIndex, Program& program )
{
  ...
  DALI_ASSERT_DEBUG( mDataProviders && mDataProviders->GetUniformMap() && "No Uniform map data provider available" );

  const UniformMapDataProvider* uniformMapDataProvider = mDataProviders->GetUniformMap();
...
void NewRenderer::BindTextures(
  ...
  Program& program,
  const DataProviders::Samplers& samplers )
{
  // @todo MESH_REWORK Write a cache of texture units to commonly used sampler textures
  unsigned int textureUnit = 0;

  for( DataProviders::Samplers::Iterator iter = samplers.Begin();
       iter != samplers.End();
       ++iter )
  {
