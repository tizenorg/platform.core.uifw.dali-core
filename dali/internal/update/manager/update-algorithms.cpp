...
inline void UpdateNodeTransformValues( Node& node, int& nodeDirtyFlags, BufferIndex updateBufferIndex )
{
  ...
  {
    TxManager::GetTxManager().SetTransform( node.GetTxId(), node.GetPosition( updateBufferIndex), node.GetScale(updateBufferIndex),
                             node.GetSize(updateBufferIndex), node.GetOrientation(updateBufferIndex), node.GetParentOrigin(), node.GetAnchorPoint() );
  }
}
