...
void NewTexture::Initialize(Context& context)
{
  ...
  PixelFormatToGl( mFormat, pixelDataType, internalFormat );

  unsigned char data[] =  { 255u, 255u, 255u ,    0u,    0u,   0u,
  ...
  if( mType == TextureType::TEXTURE_2D )
  {
    ...
    context.TexImage2D(GL_TEXTURE_2D, 0, internalFormat, 2u, 2u, 0, internalFormat, pixelDataType, data );
...
{
  return PixelFormatHasAlpha( mFormat );
}
