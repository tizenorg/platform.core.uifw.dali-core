...
void Animation::Update(BufferIndex bufferIndex, float elapsedSeconds, bool& looped, bool& finished )
{
  ...
  if( mLoopCount < 0 )
  {
    ...
    // don't increment mPlayCount as event loop tracks this to indicate animation finished (end of all loops)
  }
  else if( mCurrentLoop < mLoopCount - 1) // '-1' here so last loop iteration uses play once below
  {
    ...
    {
      ++mCurrentLoop;
      // don't increment mPlayCount until the finshed final loop
    }
  }
  else
  {
