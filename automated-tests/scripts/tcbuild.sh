...
function build {
    if [ -n "$1" ]; then
        if [ "$1" == "Cts" ]; then
            (cd Cts; ../scripts/tcheadgen.sh Cts-dali-core.h)
            if [ $? -ne 0 ]; then echo "Aborting..."; exit 1; fi
            ...
            (cd src/$2; ../../scripts/tcheadgen.sh tct-$2-core.h)
            if [ $? -ne 0 ]; then echo "Aborting..."; exit 1; fi
            cp packaging/core-$2-tests.spec ../packaging
            cp .gitignore-without-autogenerated-files .gitignore
            gbs build -A armv7l --spec core-$2-tests.spec --include-all --keep-packs | \
                tee build.log | stdbuf -o0 sed -e 's/error:/\x1b[1;91m&\x1b[0m/' \
                                               -e 's/warning:/\x1b[93m&\x1b[0m/'
            rm ../packaging/core-$2-tests.spec
            cp .gitignore-with-autogenerated-files .gitignore
        fi ## if [ "$1" -eq "Cts" ]
    else
...
function inst {
    if [ "$1" == "utc" ]; then
	    ...
	    if [ $mod != 'common' ] && [ $mod != 'manual' ]; then
           ...
           # scripts/tcpackageslistsgen.sh $MOD_NAME /opt/tct/manager/plan/Full_test.xml
           # if [ $? -ne 0 ]; then echo "Aborting..."; exit 1; fi
	echo "Cts Installation Task finished successfully"
    fi
}
