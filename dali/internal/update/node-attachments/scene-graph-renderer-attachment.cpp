...
void RendererAttachment::DoPrepareRender( BufferIndex updateBufferIndex )
{
  ...
  if( mResendDataProviders )
  {
    ...
    unsigned int* slot = mSceneController->GetRenderQueue().ReserveMessageSlot( mSceneController->GetRenderQueue().GetSceneGraphBuffers()->GetRenderBufferIndex(), sizeof( DerivedType ) );
