...
inline T CubicInterpolate( const T& p0, const T& p1, const T&  p2, const T&  p3, float progress )
{
  ...
  return a3*progress*progress*progress + a2*progress*progress + a1*progress + p1;
}

template <>
inline int CubicInterpolate( const int& p0, const int& p1, const int&  p2, const int&  p3, float progress )
{
  float a3 = p3*0.5f - p2*1.5f + p1*1.5f - p0*0.5f;
  float a2 = p0 - p1*2.5f + p2*2.0f - p3*0.5f;
  float a1 = (p2-p0)*0.5f;

  return static_cast<int>( a3*progress*progress*progress + a2*progress*progress + a1*progress + p1 );
}
