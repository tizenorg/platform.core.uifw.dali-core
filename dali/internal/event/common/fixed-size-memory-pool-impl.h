...
public:
   ...
   */
  static FixedSizeMemoryPoolPtr New( size_t size );
   ...
   * @return Return the newly allocated memory pool object
   */
  static FixedSizeMemoryPoolPtr New( size_t size, size_t initialCapacity, size_t maximumBlockCapacity );
...
private:
   ...
   * @param maximumBlockCapacity The maximum size that a new block of memory can be allocated
   */
  explicit FixedSizeMemoryPool( size_t fixedSize, size_t initialCapacity = 32, size_t maximumBlockCapacity = 1048576 );
...
private:
  ...
  size_t mFixedSize;                ///< The size of each allocation in bytes

  Block mMemoryBlocks;              ///< Linked list of allocated memory blocks
  size_t mMaximumBlockCapacity;     ///< The maximum allowed capacity of allocations in a new memory block

  Block* mCurrentBlock;             ///< Pointer to the active block
  size_t mCurrentBlockCapacity;     ///< The maximum number of allocations that can be allocated for the current block
  size_t mCurrentBlockSize;         ///< The number of allocations allocated to the current block
