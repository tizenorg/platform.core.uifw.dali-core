...
protected: // for Derived classes
  ...
  /**
   * Returns the next power of two of the given number.
   *
   * @param[in] x The number to get the next power of two.
   *
   ...
   *
   * @parameter[in] destination Pointer to the destination address.
   * @parameter[in] source Pointer to the source address.
   * @parameter[in] numberOfBytes The number of bytes to be copied.
   */
...
protected: // API for deriving classes
  ...
  void Insert( char* at, char* from, char* to, SizeType elementSize )
  {
    ...
    if( newCount > Capacity() )
    {
      ...
      // need more space
      Reserve( NextPow2( newCount ), elementSize ); // reserve enough space to store at least the next power of two elements of the new required size.
...
public: // API
   ...
   * become invalid.
   *
   * @pre Iterator \e at must be in range (Vector::Begin(), Vector::End()).
